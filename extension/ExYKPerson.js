		...
		game.broadcastAll(function(player,num,nature){
			if(player.witchcraft_point>=player.witchcraftDiv.divBg.maxNum){
				setTimeout(function(){
					...
					node.style.cssText='height:'+player.node.avatar.offsetHeight+'px;width:'+player.node.avatar.offsetWidth+'px;left:calc(3.45% + '+player.offsetLeft+'px);top:calc(7.27% + '+player.offsetTop+'px);background-size:cover;background-position:center;transition:all 1s;z-index:999;';
			...
			lib.skill.ykxuyan_mark={
				...
				content:()=>{
					...
					if(trigger.name!='phase'){
						...
						if(player.storage.ykxuyan_mark_source.ykdingze_rule.length<4&&player.storage.ykxuyan_mark_source.ykCheckConsume('Mp',((lib.config['yk_ykdingze_rank']||0)>=1?130:150))) player.storage.ykxuyan_mark_source.chooseControl('♥','♦','♣','♠','取消',function(event,player){
							return ['♥','♦','♣','♠'].randomGet();
