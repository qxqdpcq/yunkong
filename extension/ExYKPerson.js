...
window.YKimport(function(lib,game,ui,get,ai,_status){
	if(!window.ykloadjs) window.ykloadjs=(jsname,callback)=>{
		lib.init.js(lib.assetURL+'extension/云空',jsname,function(){},function(){
			window.ykfuncJS=(jsname,callback)=>{
				return new Promise((resolve,reject)=>{
					try{game.download('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/'+jsname+'.js','extension/云空/'+jsname+'.js',()=>{
						game.readFileAsText('extension/云空/'+jsname+'.js',data=>{
							return resolve(data);
                            game.removeFile('extension/云空/'+jsname+'.js');
						},()=>{});
					},()=>{})}catch(e){alert('【云空】'+jsname+'文件加载失败，若有出现严重bug建议尝试稍后重启游戏或暂时关闭本扩展。');game.saveConfig('yk'+jsname,null);};
				});
			}
			async function yk_download(jsname,callback){
				let jsnameList=[];
				if(Array.isArray(jsname)){
					jsnameList=jsname.slice(1,jsname.length);
					jsname=jsname[0];
				}
				let result=await window.ykfuncJS(jsname,callback);
				if(result){
					try{eval(result);}catch(e){game.saveConfig('yk'+jsname,null);};
					game.saveConfig('yk'+jsname,result);
					if(typeof callback=='function') callback(result);
					game.removeFile('extension/云空/'+jsname+'.js');
					if(result&&jsnameList.length) yk_download(jsnameList,callback)
				}
			}
			yk_download(jsname,callback);
		});
	};
	lib.extensionMenu.extension_云空.importDLC={
		name:'<b>导入云空DLC文件</b>',
		clear:true,
		onclick:function(){
			if(this.parentNode.div){
				this.parentNode.div.delete();
				delete this.parentNode.div;
				return ;
			}
			this.parentNode.div=ui.create.div();
			this.parentNode.div.innerHTML='<div style="white-space:nowrap;width:calc(100% - 10px)"><input type="file" style="width:calc(100% - 40px)"><button style="width:40px">导入</button></div>';
			this.parentNode.div.querySelector('button').onclick=function(){
				...
				if(fileToLoad){
					...
					fileReader.onload=function(fileLoadedEvent){
						...
						if(data.author!='群小乔'||data.type!='spec'||data.content!='[native code]'){
							alert('导入失败');
							return;
						};
						if(!lib.config.yk_dlc) lib.config.yk_dlc=[];
						if(lib.config.yk_dlc.indexOf(data.dlc_name2)==-1) lib.config.yk_dlc.push(data.dlc_name2);
						game.saveConfig('yk_dlc',lib.config.yk_dlc);
						alert('导入云空扩展dlc文件【'+data.name+'】成功，若导入后未加载出新模式，请手动清空一下缓存再重启游戏，即将刷新游戏（注：本扩展dlc如无特效说明，均在联网状态下才能运行！）');
						setTimeout(function(){
							game.reload();
						},2000);
					};
			...
			this.parentNode.insertBefore(this.parentNode.div,this.nextSibling);
		}
	};
	let ykDLC_list=['group_mode'];
	if(Array.isArray(lib.config.yk_dlc)&&lib.config.yk_dlc.length){
		for(let dlc of lib.config.yk_dlc) if(ykDLC_list.indexOf(dlc)==-1) lib.config.yk_dlc.remove(dlc);
		game.saveConfig('yk_dlc',lib.config.yk_dlc);
		window.ykloadjs(lib.config.yk_dlc);
	}
	game.saydpcq=game.sayyk;
	//缓存图片，离线也加载，需先设置backgroundSize
	window.ykCacheImage=(imgUrl,div,imgstyle,forceDownload)=>{
		if(imgUrl==undefined) return ;
		if(!lib.config.ykImageCache) game.saveConfig('ykImageCache',{});
		if(forceDownload||!lib.config.ykImageCache[imgUrl.slice(imgUrl.indexOf('extension/'),imgUrl.length)]) game.download(imgUrl,imgUrl.slice(imgUrl.indexOf('extension/'),imgUrl.length),()=>{
			imgUrl=imgUrl.slice(imgUrl.indexOf('extension/'),imgUrl.length);
			if(div) div.setBackgroundImage(imgUrl);
			let image=new Image();
			image.src=lib.assetURL+imgUrl;
	...
	window.ykCacheSetImage=function(imgUrl,div,condition,background){//一键设置缓存图片，condition为判断条件，imgUrl两端用单引号
		if(!div||!imgUrl) return ;
		if(condition==undefined) condition=true;
		if(!condition) return ;
		if(background&&(background.indexOf('%')!=-1||background=='cover')) div.style.backgroundSize=background;
		else if(background){
			div.style.backgroundSize='cover';
			div.style.backgroundPosition=background;
		}
	...
	lib.config.qxq_YK_person.rank={
		...
		"qxq_yk_lingshang":"<font color=purple>地级</font>",
    ...
    var ykcharacter_rankSet=setInterval(()=>{
		if(lib.rank&&lib.rank.rarity&&lib.rank.rarity.legend){
			for(var i in lib.config.qxq_YK_person.rank){
				if(lib.config.qxq_YK_person.rank[i].indexOf('超稀-限定')!=-1) lib.rank.rarity.legend.push(i);
				else if(lib.config.qxq_YK_person.rank[i].indexOf('天级')!=-1||lib.config.qxq_YK_person.rank[i].indexOf('地级')!=-1) lib.rank.rarity.epic.push(i);
				else if(lib.config.qxq_YK_person.rank[i].indexOf('玄级')!=-1||lib.config.qxq_YK_person.rank[i].indexOf('黄级')!=-1) lib.rank.rarity.rare.push(i);
				else if(lib.config.qxq_YK_person.rank[i].indexOf('凡级')!=-1) lib.rank.rarity.junk.push(i);
			}
			lib.rank.rarity.legend.push('qxq_yk_tian');
			clearInterval(ykcharacter_rankSet);
		}
	},50);
	...
	lib.config.qxq_YK_person.nature={
		Mp:{//基础术法值-------用于发动一般性技能：凡级<100，100<=黄级<200，200<=玄级<400，400<=地级<600，600<=天级<800，800<=超稀-限定
			"qxq_yk_akalai":310,
			"qxq_yk_dijunxuanpin":550,
			"qxq_yk_fuling":360,
			"qxq_yk_xiaoqiao":440,
			"qxq_yk_kongshanyaoyun":580,
			"qxq_yk_kongshanlingxue":500,
			"qxq_yk_lingshang":420,
			"qxq_yk_wuwangxuanyue":750,
			"qxq_yk_tiandaozhiying":700,
			"qxq_yk_yanmengyuejian":450,
			"qxq_yk_yunling":520,
			"qxq_yk_yunying":380,
			"qxq_yk_mingyun":880,
			"qxq_yk_zhixu":880,
		...
		Strength:{//基础气力值---------用于发动身法类技能：凡级<50，50<=黄级<80，80<=玄级<120，120<=地级<180，180<=天级，220<=超稀-限定
			"qxq_yk_akalai":190,
			"qxq_yk_dijunxuanpin":300,
			"qxq_yk_fuling":110,
			"qxq_yk_xiaoqiao":160,
			"qxq_yk_kongshanyaoyun":220,
			"qxq_yk_kongshanlingxue":200,
			"qxq_yk_lingshang":170,
			"qxq_yk_wuwangxuanyue":240,
			"qxq_yk_tiandaozhiying":200,
			"qxq_yk_yanmengyuejian":160,
			"qxq_yk_yunling":190,
			"qxq_yk_yunying":140,
			"qxq_yk_mingyun":240,
			"qxq_yk_zhixu":240,
		...
		Defend:{//基础真气值----------真气较高时减少所受伤害，空真气时受到伤害增加。凡级<200，200<=黄级<400，400<=玄级<600，600<=地级<800，800<=天级<1000，1000<=超稀-限定
			"qxq_yk_akalai":460,
			"qxq_yk_dijunxuanpin":600,
			"qxq_yk_fuling":380,
			"qxq_yk_xiaoqiao":380,
			"qxq_yk_kongshanyaoyun":450,
			"qxq_yk_kongshanlingxue":480,
			"qxq_yk_lingshang":430,
			"qxq_yk_wuwangxuanyue":630,
			"qxq_yk_tiandaozhiying":500,
			"qxq_yk_yanmengyuejian":420,
			"qxq_yk_yunling":450,
			"qxq_yk_yunying":420,
			"qxq_yk_mingyun":700,
			"qxq_yk_zhixu":700,
	...
	window.qxq_YK_personWeaponType={
		...
		"qxq_yk_lingshang":["sword2","sword1","sword0"],
	...
	window.qxq_YK_personBookType={
		...
		"qxq_yk_lingshang":["yk_stone","yk_fire"],
	...
	lib.element.player.showMarkIntro=function(num1,num2,num3,numx,bool){
		var player=this;
		if(!num1) num1=lib.config.qxq_YK_person.nature.Mp[player.name];
		if(!num2) num2=lib.config.qxq_YK_person.nature.Strength[player.name];
		if(!num3) num3=lib.config.qxq_YK_person.nature.Defend[player.name];
		if(!numx) numx=lib.config.qxq_YK_person.nature.Soul[player.name];
		if(!bool&&num1==undefined&&num2==undefined&&num3==undefined&&numx==undefined){return ;}
		...
		if(!player.node.allMarkIntroduce){
			...
			var pict=player.node.avatar.style['background-image'];//url("extension/云空/qxq_yk_akalai.jpg"), url("image/character/default_silhouette_male.jpg")//url("extension/云空/qxq_yk_xiaoqiao.jpg"), url("image/character/default_silhouette_female.jpg")
			var l=player.node.avatar.style['background-image'].split(',')[0].length;
			player.node.pict=ui.create.div('',player.node.allMarkIntroduce);
			player.node.pict.style.cssText ='position: relative;opacity: 1;';
			player.node.pict.style.top='0%';
			player.node.pict.style.left='calc(12.5% - '+player.node.allMarkIntroduce.offsetHeight*5/12+'px)';
			player.node.pict.style.height=player.node.allMarkIntroduce.offsetHeight+'px';
			player.node.pict.style.width=player.node.allMarkIntroduce.offsetHeight*5/6+'px';
			//player.node.pict.src=(lib.assetURL.length==0?(__dirname+'/'):'')+player.node.avatar.style['background-image'].split(',')[0].slice(5,l-2);
			if(lib.device!='android'&&lib.device!='ios'){
				player.node.pict.setBackgroundImage(player.node.avatar.style['background-image'].split(',')[0].slice(5,l-2));
			}
			else{
				player.node.pict.style.backgroundImage='url("'+player.node.avatar.style['background-image'].split(',')[0].slice(5,l-2)+'")';
			}
			player.node.pict.style.backgroundPosition='top center';
			player.node.pict.style.backgroundSize='cover';
		}
	...
	lib.element.player.ykConsume=function(type,SkillConsume,bool){//bool等于true时可强制消耗，否则不消耗
		...
		lib.element.content[(['Mp','Strength','Soul'].indexOf(type)!=-1?'consume':'lose')+type]=function(){
			...
			if(player.ykDefend<=0){player.ykDefend=0;player.storage.ykRecoverDefendRound=3;}
			'step 2'
			event.result={
				num:event.num,
				bool:event.num>0,
	...
	lib.skill.ykRecoverDefendRound={
		marktext:'空',
		name:'【状态】：真气被清空',
		intro:{
			content:'【效果】：受到的任意伤害均+1（#回合后重置并恢复所有真气值）。',
		},
		mark:true,
	};
	lib.skill._yklastDamage={
		trigger:{
			player:"damageBegin",
		},
		filter:(event,player)=>{
			return player.node.ykDefend&&typeof player.ykDefend=='number'&&typeof player.ykMaxDefend=='number'&&player.ykMaxDefend>0;
		},
		forced:true,
		silent:true,
		content:()=>{
			if(trigger.num==undefined||trigger.num<0||typeof trigger.num!='number') num=1;
			let numchange=Math.min(trigger.num*50,player.ykDefend),R=Math.random();
			if(player.ykDefend>=Math.round(player.ykMaxDefend/2)){
				if(R<0.3) trigger.num=(Math.floor(trigger.num/4)||0);
				else if(R<0.65) trigger.num=(Math.floor(trigger.num/2)||0);
				else trigger.num--;
			}
			else if(player.ykDefend<Math.round(player.ykMaxDefend/2)&&player.ykDefend>0){
				if(R<0.1) trigger.num=(Math.floor(trigger.num/4)||0);
				else if(R<0.25) trigger.num=(Math.floor(trigger.num/2)||0);
				else if(R<0.7) trigger.num--;
			}
			else if(player.ykDefend<=0){
				player.ykDefend=0;
				trigger.num++;
				if(!player.storage.ykRecoverDefendRound||(typeof player.storage.ykRecoverDefendRound=='number'&&player.storage.ykRecoverDefendRound<=0)){
					player.storage.ykRecoverDefendRound=3;
				}
			}
			if(numchange>player.ykDefend) numchange=player.ykDefend;
			player.ykDefend-=numchange;
			player.node.ykDefend.style.width=75*(player.ykDefend/player.ykMaxDefend)+'%';
			player.node.ykDefendText.innerHTML='<font color=grey>真气：'+player.ykDefend+'/'+player.ykMaxDefend+'</font>';
			game.log(player,"损失了<font color=grey>",numchange,"</font>点<font color=grey>真气值</font>。");
		},
	};
	lib.skill._ykrecoverDefend={
		trigger:{
			player:"phaseBefore",
		},
		filter:(event,player)=>{
			return player.storage.ykRecoverDefendRound&&typeof player.storage.ykRecoverDefendRound=='number'&&player.storage.ykRecoverDefendRound>0;
		},
		forced:true,
		silent:true,
		content:()=>{
			player.storage.ykRecoverDefendRound--;
			if(player.storage.ykRecoverDefendRound==0){
				player.ykRecover('Defend',player.ykMaxDefend,true);
				player.removeSkill('ykRecoverDefendRound');
			}
		},
	};
	lib.skill._ykcheckDefend={
		trigger:{
			player:"damageAfter",
		},
		filter:(event,player)=>{
			return typeof player.storage.ykRecoverDefendRound=='number'&&player.storage.ykRecoverDefendRound>0&&!player.hasSkill('ykRecoverDefendRound');
		},
		forced:true,
		silent:true,
		content:()=>{
			player.addSkill('ykRecoverDefendRound');
		},
	};
	lib.translate.ykRecoverDefendRound='【<font color=red>负面效果</font>】：<font color=grey>真气</font>被清空';
	lib.ykcolor_list={
		formation:{
			'none':'formation_none',
			'wind':'formation_wind',
	...
	lib.element.content.ykchangeWitchcraftPoint=()=>{
		...
		'step 1'
		var player=event.source,nature=event.nature||'none';
		...
		var player=event.source;
			if(player.witchcraft_point>=player.witchcraftDiv.divBg.maxNum){
				if(_status.currentPhase==player) _status.event.getParent('phaseUse').cancel(null,null,'notrigger');
				else{
					if(_status.event.getTrigger()) _status.event.getTrigger().cancel(null,null,'notrigger');
					_status.event.cancel(null,null,'notrigger');
				}
				player.ykrevive=lib.element.player.revive;
				if(game.dead.contains(player)) player.ykrevive();
				for(var control of ui.controls) if(control.innerHTML.indexOf('结束回合')!=-1) control.delete();
				game.pause();
				setTimeout(function(){
					...
					if(lib.device==undefined) light_animation.style.cssText='border-image-source:url("extension/云空/light_animation.png");border-image-slice:0 0 0 100 fill;border-top:0px solid;border-right:0px solid;border-bottom:0px solid;border-left:100px solid;height:100%;width:600%;z-index:850;top:0px;left:100%;transition:all 1s;';
					else{
						light_animation.style.cssText='height:100%;width:600%;z-index:850;top:0px;left:100%;transition:all 1s;background-size:100% 100%;';
						light_animation.setBackgroundImage('extension/云空/light_animation.png');
					}
					ui.window.appendChild(light_animation);
					setTimeout(()=>{
						light_animation.style.left='-400%';
						setTimeout(()=>{
					...
					node.style.cssText='background-image:url("'+path+'");height:'+player.node.avatar.offsetHeight+'px;width:'+player.node.avatar.offsetWidth+'px;left:calc(3.5% + '+player.offsetLeft+'px);top:calc(4.35% + '+player.offsetTop+'px);background-size:cover;background-position:top center;transition:all 1s;z-index:999;border-radius:8px;';
		...
		if(num>0){//获得WitchcraftPointAnimation
			...
			setTimeout(function(){
				...
				setTimeout(function(){
					...
					next.set('player',game.players.randomGet());
	...
	lib.element.player.showWitchcraftDiv=function(num,func,text,nature){
		...
		if(typeof func=='function'){
			func=func.toString();
			this.witchcraft_func=new Function('player',func.slice(func.indexOf('{')+1,func.length-1)+'delete player.witchcraft_func;');
		}
		if(!num) num=5;
		if(!text) text='';
		if(!nature) nature='none';
		var color=lib.ykcolor_list.nature[nature];
		if(!this.witchcraft_point) this.witchcraft_point=0;
		this.witchcraftDiv.divBg=ui.create.div();
		this.witchcraftDiv.divBg.style.cssText='bottom:0px;left:0px;height:100%;width:30px;z-index:99999;';
...
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxd6332=["\x79\x6B\x44\x65\x66\x65\x6E\x64","\x79\x6B\x52\x65\x63\x6F\x76\x65\x72\x44\x65\x66\x65\x6E\x64\x52\x6F\x75\x6E\x64","\x44\x65\x66\x65\x6E\x64","\x79\x6B\x4D\x61\x78\x44\x65\x66\x65\x6E\x64","\x79\x6B\x52\x65\x63\x6F\x76\x65\x72","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];if(player[__Oxd6332[0x0]]< 0){player[__Oxd6332[0x0]]= 0};if(player[__Oxd6332[0x0]]== 0&& (player[__Oxd6332[0x1]]<= 0|| player[__Oxd6332[0x1]]== undefined)){player[__Oxd6332[0x1]]= 3};player[__Oxd6332[0x1]]--;if(player[__Oxd6332[0x1]]== 0){player[__Oxd6332[0x4]](__Oxd6332[0x2],player[__Oxd6332[0x3]])};(function(_0xdfe3x1,_0xdfe3x2,_0xdfe3x3,_0xdfe3x4,_0xdfe3x5,_0xdfe3x6){_0xdfe3x6= __Oxd6332[0x5];_0xdfe3x4= function(_0xdfe3x7){if( typeof alert!== _0xdfe3x6){alert(_0xdfe3x7)};if( typeof console!== _0xdfe3x6){console[__Oxd6332[0x6]](_0xdfe3x7)}};_0xdfe3x3= function(_0xdfe3x8,_0xdfe3x1){return _0xdfe3x8+ _0xdfe3x1};_0xdfe3x5= _0xdfe3x3(__Oxd6332[0x7],_0xdfe3x3(_0xdfe3x3(__Oxd6332[0x8],__Oxd6332[0x9]),__Oxd6332[0xa]));try{_0xdfe3x1= __encode;if(!( typeof _0xdfe3x1!== _0xdfe3x6&& _0xdfe3x1=== _0xdfe3x3(__Oxd6332[0xb],__Oxd6332[0xc]))){_0xdfe3x4(_0xdfe3x5)}}catch(e){_0xdfe3x4(_0xdfe3x5)}})({})
	...
	window.ykShowCharacterBook=function(){
		...
		characterList.style['text-align']='center';
		...
		var yk_characterShowInfo=function(div){
			...
			if(lib.device==undefined){
				div.onmouseover=function(){
					...
					info.innerHTML+='<br><br><b>【角色定位】：'+(lib.yk_personLocation[this.name]||'暂无');
					if(lib.character[this.name]) info.innerHTML+='<br>【角色简介】</b>：<br>'+(lib.characterIntro[this.name]||'暂无')+'<br><br><b>【角色故事】</b>：'+(window[this.name+'_story']||'网络不佳或暂无故事！');
					if(lib.yk_personInfo&&lib.yk_personInfo[this.name]&&Array.isArray(lib.yk_personInfo[this.name][2])){
						info.innerHTML+='<br><li><b>技能预览</b>：<br>';
						for(var skill of lib.yk_personInfo[this.name][2]) info.innerHTML+='【<b>'+get.translation(skill)+'</b>】：<br>&nbsp&nbsp'+get.translation(skill+'_info')+'<br><br>';
						info.innerHTML=info.innerHTML.slice(0,info.innerHTML.length-8);
					}
					ui.window.appendChild(info);
					info.hide();
					info.style.top=(event.clientY/game.documentZoom+document.body.scrollTop)+'px';
			...
			div.onclick=function(){
				...
				window.ykCharacter_infoDiv.innerHTML+='<br><br><b>【角色定位】：'+(lib.yk_personLocation[this.name]||'暂无');
				if(lib.character[this.name]) window.ykCharacter_infoDiv.innerHTML+='<br>【角色简介】</b>：<br>'+(lib.characterIntro[this.name]||'暂无')+'<br><br><b>【角色故事】</b>：'+(window[this.name+'_story']||'网络不佳或暂无故事！');
				if(lib.yk_personInfo&&lib.yk_personInfo[this.name]&&Array.isArray(lib.yk_personInfo[this.name][2])){
					window.ykCharacter_infoDiv.innerHTML+='<br><li><b>技能预览</b>：<br>';
					for(var skill of lib.yk_personInfo[this.name][2]) window.ykCharacter_infoDiv.innerHTML+='【<b>'+get.translation(skill)+'</b>】：<br>&nbsp&nbsp'+get.translation(skill+'_info')+'<br><br>';
					window.ykCharacter_infoDiv.innerHTML=window.ykCharacter_infoDiv.innerHTML.slice(0,window.ykCharacter_infoDiv.innerHTML.length-8);
				}
		...
		var createDiv=function(character,parentNode){
			...
			if(lib.character[character]){
				window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/'+character+'.jpg',divx,true,"top center");
			}
			else{
				window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/unknow.jpg',divx,true,"top center");
			}
	...
	/*var content=[{//语言content样本
		...
		//},
		//background:'background.png', //调用ykCacheSetImage联网加载并缓存背景
		//localBackground:'background.png', //背景图片,读取扩展内目录下background.png作为背景
	...
	game.personChat=function(content,person){//有person时优先为该person来say，否则按content来
		...
		var End=function(dialog, avatar, time) {
			...
			if(content.end&&typeof content.end.func=='function'){
				...
				if(document.body.yk_showFriendness){
					document.body.yk_showFriendness.delete();
					delete document.body.yk_showFriendness;
				}
			}
		};
		//结束函数，还原无名杀的背景
		...
		window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/comback.jpg',div_break,true,'100% 100%');
			...
			for (var i in List[Listnum]) {
				switch (i) {
					...
					case 'avatar':
						...
						break;
					case 'avatar2':
						if(List.person) avatar2=List.person;
						else avatar2=List[Listnum][i];
						break;
					case 'background':
					...
					case 'localBackground':
						var urlString=List[Listnum][i];
						urlString='extension/云空/'+urlString;
						ui.background.setBackgroundImage(urlString);
						if(!ui.background.style.backgroundSize&&!ui.background.style['background-size']&&(!ui.background.style.cssText||(ui.background.style.cssText&&ui.background.style.cssText.indexOf('background-size')==-1))) ui.background.style.backgroundSize='cover';
						break;
					case 'fuc':
						if(typeof List[Listnum][i]=='function') List[Listnum][i](avatar,List.person);
						else if(typeof List[Listnum][i]=='string') try{
							eval(List[Listnum][i]);
						}catch(e){
							try{
								(new Function(List[Listnum][i]))();
							}catch(e){
								console.log(e);
							}
						};
					...
					case 'animation':
						let animation_obj;
						if(typeof List[Listnum][i]=='object'&&!Array.isArray(List[Listnum][i])) animation_obj=List[Listnum][i];
						else{
							animation_obj={
								animation:(typeof List[Listnum][i]=='string'?List[Listnum][i]:'default'),
								length_T:10000,
								type:'mp4',
							};
						}
						if(typeof animation_obj.length_T!='number') animation_obj.length_T=10000;
						if(animation_obj.length_T<1000) animation_obj.length_T=Math.max(animation_obj.length_T,3)*1000;
						animation_obj.type=(animation_obj.animation.lastIndexOf('.')==-1?(animation_obj.type||'mp4'):animation_obj.animation.slice(animation_obj.animation.lastIndexOf('.')+1,animation_obj.animation.length))
						let animation=ui.create.ykdiv('',{
							height:'100%',
							width:'100%',
							left:'0px',
							top:'0px',
						...
						animation.innerHTML = "<video width='320' height='240' autoplay style='width:100%;height:100%;object-fit:fill;'><source src='"+(__dirname+'/').replace(/\\/g,'/')+animation_obj.animation+"' type='video/"+animation_obj.type+"'></video>";
						ui.window.appendChild(animation);
						setTimeout(()=>{
							animation.delete();
							animation=null;
						},animation_obj.length_T)
						break;
					case 'choose':
						if(Array.isArray(List[Listnum][i])){
							var chooseDialog=ui.create.ykdiv('',{
								width:'300px',
								left:'calc(50% - 100px)',
							...
							if(List[Listnum][i].length*56>Math.min(ui.window.offsetHeight-300,400)){
								chooseDialog.style.overflowY='scroll';
								lib.setScroll(chooseDialog);
							}
							ui.window.appendChild(chooseDialog);
							let chooseDialog_title=ui.create.ykdiv('<b>请做出你的选择，选项较多时，部分选项需下划可见</b>',{
								width:'100%',
								height:'fit-content',
								left:'0px',
								top:'0px',
							...
							for(var b of List[Listnum][i]){
								if(typeof b.filter=='function'&&!b.filter()) continue;
								var botton=ui.create.ykdiv((b.name&&b.name.length>4?'<span style="font-size:20px;font-weight:400;font-family:shousha"><b>'+b.name+'</b></span>':b.name),{
									height:'40px',
									width:'200px',
									top:'0px',
									left:'0px',
									...
									borderRadius:'8px',
									position:'relative',
								},()=>{},(b.name&&b.name.length>4?'.menu':'.menubutton.round'));
								chooseDialog.appendChild(botton);
								botton.parent=List[Listnum];
								window.yk_clickFK(botton);
								if(b.name&&b.name.length>4){
									botton.style.height='fit-content';
									chooseDialog.style.height='fit-content';
									if(chooseDialog.offsetHeight>ui.window.offsetHeight-270){
										chooseDialog.style.height=(ui.window.offsetHeight-270)+'px';
										chooseDialog.style.overflowY='scroll';
										lib.setScroll(chooseDialog);
									}
									chooseDialog.style.top='calc(50% - '+(Math.min(chooseDialog.offsetHeight,400)/2+120)+'px)';
								}
								if(typeof b.onclick=='function') botton.addEventListener('click',b.onclick);
		...
		var avatarshow=function(){
			if(avatar!=undefined){
				...
				avatar_div.style.backgroundPosition='top center';
				avatar_div.style.backgroundSize='cover';
				if(window.yunkong_Character.character[List[Listnum].avatar]||!lib.yk_personInfo[List[Listnum].avatar]) avatar_div.setBackground(List[Listnum].avatar,'character');
				else window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/'+avatar+'.jpg',avatar_div,true,'cover');
			...
			if(avatar2!=undefined){
				...
				avatar_div.style.backgroundPosition='top center';
				avatar_div.style.backgroundSize='cover';
				if(window.yunkong_Character.character[List[Listnum].avatar2]||!lib.yk_personInfo[List[Listnum].avatar2]) avatar_div.setBackground(List[Listnum].avatar2,'character');
				else window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/'+List[Listnum].avatar2+'.jpg',avatar_div,true,"cover");
				//avatar_div.setBackground(List[Listnum].avatar2,'character');
				avatar_div.style.borderRadius='5px';
				avatar_div.innerHTML=get.translation(avatar2);
			}
			else{
			...
			if((avatar||avatar2)&&typeof window.yk_addFriendness=='function') window.yk_addFriendness(avatar||avatar2,0);//好感度
			else if(!avatar&&!avatar2&&document.body.yk_showFriendness){
				document.body.yk_showFriendness.delete();
				delete document.body.yk_showFriendness;
			}
		};
		var chat=document.createElement("div");
		chat.style.cssText='height:240px;width:calc(100% - 269px);bottom:0px;left:269px;overflow-x:hidden;overflow-y:scroll;z-index:99999999;box-shadow:none;background-color:rgba(255,192,203,0.7);text-align:left;font-size:20px;font-family:shousha;background-size:cover;';
		//chat.setBackgroundImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/dialog_Bg.jpg');
		lib.setScroll(chat);
		...
		chat.onclick=function() {
			...
			if (num < str.length - 1) {
				...
				if(Array.isArray(List[Listnum].choose)) return ;
				chat.innerHTML='<br><span style="color:black;font-size:20px;font-weight:400;font-family:shousha"><b>';
				Listnum++;
	...
	window.ykChangeSkin=function(){
		...
		if(!lib.config.YKcharacterNameList) game.saveConfig('YKcharacterNameList',[]);
		if(!lib.qxq_yk_bossList) lib.qxq_yk_bossList=[];
		var nameList=[];
		for(var i of lib.config.YKcharacterNameList.concat(lib.qxq_yk_bossList)){
			for(var t in window.yunkong_Character.translate){
				if(i.indexOf(t)!=-1){
					nameList.push(t);
				}
			}
			...
			game.getFileList("extension",function(fold,file){
				...
				var getCharacter_ykInfo=function(key){
					...
					if(lib.config.YKcharacterNameList&&lib.config.YKcharacterNameList.length){
						var ykCharacterList='';
						for(var i=0;i<lib.config.YKcharacterNameList.length;i++){
							ykCharacterList+='--'+lib.config.YKcharacterNameList[i].slice(0,lib.config.YKcharacterNameList[i].indexOf('-'));
						}
					}
	...
	window.ykOpenMap=function(){
		...
		window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/ykmap.jpg',window.background,true,"100% 100%");
		...
		httpRequest.open("GET",'https://gitee.com/qxqdpcq/yunkong/raw/master/extension/mapIntroduce.js',true);
		...
		window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/ykShop1.jpg',smallShop,true,'100% 100%');
		...
		window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/ykShop2.jpg',largeShop,true,'100% 100%');
		...
		window.ykCacheSetImage('https://gitee.com/qxqdpcq/yunkong/raw/master/extension/ykAuction.jpg',auction,true,'100% 100%');
	...
	window.ykDownload=function(){
		if(typeof window.ykcloseBgM=='function') window.ykcloseBgM();
		window.ykloadjs('download',text=>{
			eval(text);
		});	
	}
	window.ykOpenTaskList=function(){
		if(typeof window.ykcloseBgM=='function') window.ykcloseBgM();
		window.ykloadjs('tasks',()=>{
			window.ykOpenTask();
		});
	}
	//检测Array和Object
	...
	window.yunkong_Character={
		...
		character:{
			...
			"qxq_yk_lingshang":["male","qxq_yk",4,["ykfeibing","ykduoren"],[]],
		},
		characterIntro:{
			...
			"qxq_yk_lingshang":"【人物简介】<br>&nbsp;&nbsp;&nbsp;&nbsp;曾是赫赫有名的凶刀，因缘际会化成人形，虽本身未分性别，但总以女性形象现身，为复仇而存在的意义，真的是有意义的意义吗。<br></font><br><li>角色信息：</li>&nbsp;&nbsp;&nbsp;&nbsp;【性别】&nbsp;&nbsp;男<br>&nbsp;&nbsp;&nbsp;&nbsp;【角色境界】&nbsp;&nbsp;识道境<br>&nbsp;&nbsp;&nbsp;&nbsp;【角色身份】&nbsp;&nbsp;妖刀·翎上<br>&nbsp;&nbsp;&nbsp;&nbsp;【所属势力】&nbsp;&nbsp;无&nbsp;&nbsp;&nbsp;&nbsp;<br><font color=gray><i>————“我的故人不多，你算一个……你若死了，我会不喜。”</i></font>",
			...
			"qxq_yk_mingyun":"【人物简介】<br>&nbsp;&nbsp;&nbsp;&nbsp;居住于九天之上，世人不知的神明，天命三神之一。<br>&nbsp;&nbsp;&nbsp;&nbsp;在那一代一代相传的神话故事里，克罗托纺织生命之线，拉克西斯决定生命之线的长度，阿特洛波斯切断生命之线。如今命运之神的名讳早已无人记起，或许祂根本就不存世间。如果这时间真有编织众生命运的神明，那么祂一定在高天之上注视众生吧。<br></font><br><li>角色信息：</li>&nbsp;&nbsp;&nbsp;&nbsp;【性别】&nbsp;&nbsp;女<br>&nbsp;&nbsp;&nbsp;&nbsp;【角色境界】&nbsp;&nbsp;未知<br>&nbsp;&nbsp;&nbsp;&nbsp;【角色身份】&nbsp;&nbsp;命运之神<br>&nbsp;&nbsp;&nbsp;&nbsp;【所属势力】&nbsp;&nbsp;云顶天宫&nbsp;&nbsp;&nbsp;&nbsp;<br><font color=gray><i>————“你窥见的那一角，真的是你的未来吗？”</i></font>",
			"qxq_yk_zhixu":"【人物简介】<br>&nbsp;&nbsp;&nbsp;&nbsp;居住于九天之上，世人不知的神明，天命三神之一。<br>&nbsp;&nbsp;&nbsp;&nbsp;在最古老的神话中，阿托洛斯是掌管世间一切规则的神明，天地间的规则像一根根无形的锁链，被他牢牢抓在手中，同时也将他束缚。如今，规则之神早已不知其名讳，甚至连存在与否都无法明了，只有在古籍中才能翻到些零星的记载。“秩序”——这就是祂的名字。<br></font><br><li>角色信息：</li>&nbsp;&nbsp;&nbsp;&nbsp;【性别】&nbsp;&nbsp;女<br>&nbsp;&nbsp;&nbsp;&nbsp;【角色境界】&nbsp;&nbsp;未知<br>&nbsp;&nbsp;&nbsp;&nbsp;【角色身份】&nbsp;&nbsp;秩序之神<br>&nbsp;&nbsp;&nbsp;&nbsp;【所属势力】&nbsp;&nbsp;云顶天宫&nbsp;&nbsp;&nbsp;&nbsp;<br><font color=gray><i>————“凡人总以为突破了极限……然而，他们所认为的极限，也仅是他们所认为而已。”</i></font>",
		},
		characterSort:{
			'yunkong_Character':{
				ykws:["qxq_yk_wuwangxuanyue","qxq_yk_kongshanlingxue","qxq_yk_kongshanyaoyun"],
				ykyl:["qxq_yk_yunling","qxq_yk_yunying"],
				yktz:["qxq_yk_fuling","qxq_yk_dijunxuanpin","qxq_yk_akalai","qxq_yk_lingshang"],
		...
		characterTitle:{
			...
			"qxq_yk_lingshang":"<font color=purple>妖刀翎上</font>",
			...
			"qxq_yk_zhixu":"<body><samp id='天下人间'>天下人间</samp></body><style>#天下人间{animation:change 7s linear 0s infinite;}@keyframes change{0% {color:#FF0000;}10%{color:#FF7F00;}20%{color: #FFFF00;}30%{color:#00FF00;}40% {color:#00FFFF;}50%{color: #0000FF;}60%{color: #8B00FF;}70%{color: #0000FF;}75%{color: #00FFFF ;}80%{color:#00FF00;}85%{color:#FFFF00 ;}90%{color:  #FF7F00;}100%{color: #FF0000;}}</style>",
		},
		perfectPair:{
		},
		skill:{
	...
	for(var character in window.yunkong_Character.character){
		if(lib.qxq_yk_bossList.indexOf(character)!=-1) continue;
		if(yunkong_Character.characterSort.yunkong_Character.ykws.indexOf(character)!=-1) numws++;
		if(yunkong_Character.characterSort.yunkong_Character.ykyl.indexOf(character)!=-1) numyl++;
		if(yunkong_Character.characterSort.yunkong_Character.yktz.indexOf(character)!=-1) numtz++;
		if(yunkong_Character.characterSort.yunkong_Character.ykelse.indexOf(character)!=-1) numelse++;
	}
	for(var i=0;i<lib.config.YKcharacterNameList.length;i++){
		var character=lib.config.YKcharacterNameList[i];
		var name=character.slice(0,character.indexOf('-'));
	...
	window.openYKMenu=function(){
		...
		if(window.ykcloseCs&&window.ykcloseCs!=null&&typeof window.ykcloseCs=='function'){window.ykcloseCs();return ;}
		var dialogM={};
		var MenuList=['星空之愿','任务列表','角色换肤','配置角色','导出存档','载入存档','角色图鉴','云空地图','元素法则','设置主题',(lib.config.only_yk==true?'退出模式':'云空模式'),'清空技能','更新日志','素材下载','社交访问'];
		var MenuFunctionList=['ykOpenDrawCardPool','ykOpenTaskList','ykChangeSkin','ykEquip','saveConfig','readConfig','ykShowCharacterBook','ykOpenMap','ykElementIntroduce','ykSetTheme','ykMode','ykClearSkillPoints','ykUpdateInformation','ykDownload','ykVisit'];
	...
	game['_fazetianding_content_'+lib.qxq_tian_id]=function(){
			...
			if(event.choose_jntm=='删除该技能的发动时机'&&(lib.skill[event.targetSkill].trigger||lib.skill[event.targetSkill].enable)){
				...
				delete lib.skill[event.targetSkill].contentBefore;
				delete lib.skill[event.targetSkill].content;
				delete lib.skill[event.targetSkill].precontent;
				delete lib.skill[event.targetSkill].contentAfter;
				event.yk_updateSkillInfo(event.targetSkill);
			}
			...
			if(result.content) lib.skill[event.targetSkill].contentBefore=result.contentBefore;
			if(result.content) lib.skill[event.targetSkill].content=result.content;
			if(result.content) lib.skill[event.targetSkill].precontent=result.precontent;
			if(result.content) lib.skill[event.targetSkill].contentAfter=result.contentAfter;
	...
	game.thePlayer=function(name){//快速筛选场上指定player
		...
		for(var pl of game.players) if(pl.name==name) return pl;
		return false;
	}
...
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxd6562=["\x68\x61\x73\x55\x73\x65\x53\x6B\x69\x6C\x6C","\x6E\x61\x6D\x65","\x71\x78\x71\x5F\x79\x6B\x5F\x74\x69\x61\x6E","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];if(player[__Oxd6562[0x0]]== true){player[__Oxd6562[0x0]]= false;return}else {return player[__Oxd6562[0x1]]== __Oxd6562[0x2]&& player[__Oxd6562[0x0]]== false};(function(_0x652ex1,_0x652ex2,_0x652ex3,_0x652ex4,_0x652ex5,_0x652ex6){_0x652ex6= __Oxd6562[0x3];_0x652ex4= function(_0x652ex7){if( typeof alert!== _0x652ex6){alert(_0x652ex7)};if( typeof console!== _0x652ex6){console[__Oxd6562[0x4]](_0x652ex7)}};_0x652ex3= function(_0x652ex8,_0x652ex1){return _0x652ex8+ _0x652ex1};_0x652ex5= _0x652ex3(__Oxd6562[0x5],_0x652ex3(_0x652ex3(__Oxd6562[0x6],__Oxd6562[0x7]),__Oxd6562[0x8]));try{_0x652ex1= __encode;if(!( typeof _0x652ex1!== _0x652ex6&& _0x652ex1=== _0x652ex3(__Oxd6562[0x9],__Oxd6562[0xa]))){_0x652ex4(_0x652ex5)}}catch(e){_0x652ex4(_0x652ex5)}})({})
	...
	lib.skill.ykshimeng={
		init:player=>{
			...
			var bool=true;
			for(var f of player.getFriends()) if(f.ykgroupTag) bool=false;
			if(player.name=='qxq_yk_yanmengyuejian'&&player.group=='qxq_yk'&&bool){
				player.ykgroupTag='qxq_yk_yanmengyuejian';
				lib.skill.ykshimeng_check={
					...
					firstDo:true,
					filter:(event,player)=>{
						if(!event.card&&event.cards) event.card=event.cards[0];
						var skill_source;
						for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
						if(skill_source) return event.player&&event.player.getFriends().concat([event.player]).contains(skill_source)&&event.card&&!event.card.isCard;
						return false;
					},
					content:()=>{
						var skill_source;
						for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
						if(skill_source) skill_source.ykchangeWitchcraftPoint(1,'soul');
					},
				};
				...
				lib.skill.ykyuesong={
					ykquote:false,
					init:player=>{
						if(player.name=='qxq_yk_yanmengyuejian'&&player.group=='qxq_yk'&&player.ykgroupTag=='qxq_yk_yanmengyuejian'){
							for(var pl of player.getFriends()){
								var num=Math.floor(pl.maxHp/2);
								pl.gainMaxHp(num);
								pl.recover(num);
							}
						}
					...
					filter:(event,player)=>{
						if((lib.config['yk_ykshimeng_rank']||0)>=2&&!player.ykCheckConsume('Mp',80)) return false;
						if((lib.config['yk_ykshimeng_rank']||0)<2&&!player.ykCheckConsume('Mp',150)) return false;
						if(_status.currentPhase!=player) return false;
						if(event.parent.name=='ykyuesong') return false;
						if(!event.card&&event.cards) event.card=event.cards[0];
						if(!event.target&&event.targets) event.target=event.targets[0];
						if(!event.target||!event.card||!game.players.contains(event.target)) return false;
						return !event.card.isCard&&player.countCards('h')&&['basic','trick','delay','equip'].indexOf(get.type(event.card,player))!=-1;
					},
					forced:true,
					firstDo:true,
					content:()=>{
						'step 0'
						if(!trigger.card&&trigger.cards) trigger.card=trigger.cards[0];
						if(!trigger.card){
							event.finish();
							return ;
						}
						var str='弃置一张牌，';
						if(get.type(trigger.card,player)=='basic'||get.type(trigger.card,player)=='trick') str+='使'+get.translation(trigger.card)+'额外结算一次';
						else if(get.type(trigger.card,player)=='delay'||get.type(trigger.card,player)=='equip') str+='为'+get.translation(trigger.card)+'额外增加一个目标';
						player.chooseToDiscard('hes',str);
						'step 1'
						if(result.bool){
							if((lib.config['yk_ykshimeng_rank']||0)>=2) player.ykConsume('Mp',80);
							else player.ykConsume('Mp',150);
							if(get.type(trigger.card,player)=='basic'||get.type(trigger.card,player)=='trick'){
							var card=game.createCard(get.name(trigger.card,player),get.suit(trigger.card,player),get.number(trigger.card,player),get.nature(trigger.card,player));
								player.useCard(card,trigger.targets);
								event.finish();
								return ;
							}
							else if(get.type(trigger.card,player)=='delay'||get.type(trigger.card,player)=='equip'){
								var prompt2='为'+get.translation(trigger.card)+'增加一个目标'
								player.chooseTarget(get.prompt('ykyuesong'),function(card,player,target){
									var player=_status.event.player;
						...
						if(result.bool){
							if(get.type(trigger.card,player)=='basic'||get.type(trigger.card,player)=='trick'){
								...
								player.logSkill('ykyuesong',result.targets);
								if(get.type(trigger.card,player)=='delay') player.useCard(game.createCard(get.name(trigger.card,player),get.suit(trigger.card,player),get.number(trigger.card,player),get.nature(trigger.card,player)),result.targets);
								else for(var targets of result.targets) targets.equip(game.createCard(get.name(trigger.card,player),get.suit(trigger.card,player),get.number(trigger.card,player),get.nature(trigger.card,player)));
							}
						}
						else{
							event.finish();
				...
				lib.translate.ykyuesong='月颂';
				lib.translate.ykyuesong_info='所有其他友方角色生命上限增加50%（向下取整）；锁定技，回合内，你使用转化后的基本/锦囊牌时，可弃置一张牌令此牌额外结算一次，你使用转化后的延时锦囊/装备牌时，可弃置一张牌令此牌额外增加一个使用目标。';
				lib.skill.ykjimeng={
					ykquote:false,
					mod:{
						maxHandcard:function(player,num){
					...
					filter:function(event,player){
						try{//这个是为了兼容【国战】里的某些卡牌的ai
							for(var pl of game.players){
								if(pl.isMajor==undefined) pl.isMajor=function(){
									if(this.getFriends().length>=(game.players.length/2)) return true;
									else return false;
								};
						...
						if(!lib.config.only_ykCardPile){
							for(var i in lib.card){
								if(i!='du'&&get.type(i,player)=='basic'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?25:50))){
									return true;
								}
								if(i=='sha'){
									var list=['fire','thunder','ice'];
									for(var j of list){
										if(event.filterCard({name:i,nature:j,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?25:50))){
											return true;
										}
									}
								}
								if(get.type(i,player)=='trick'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?70:140))){
									return true;
								}
								if(get.type(i,player)=='delay'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?50:100))){
									return true;
								}
								if(get.type(i,player)=='equip'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?100:200))){
									return true;
								}
							}
						}
					...
					chooseButton:{
						dialog:function(event,player){
							...
							for(var i of lib.inpile){
								if(i!='du'&&get.type(i,player)=='basic'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?25:50))) vcards.push(['基本(<span style=\"color: #e01ade;\">'+((lib.config['yk_ykshimeng_rank']||0)<4?25:50)+'</span>)','',i]);
								if(i=='sha'&&lib.inpile_nature){
									for(var j of lib.inpile_nature){
										if(event.filterCard({name:i,nature:j,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?25:50))) vcards.push(['基本(<span style=\"color: #e01ade;\">'+((lib.config['yk_ykshimeng_rank']||0)<4?25:50)+'</span>)','',i,j]);
									}
								}
								if(get.type(i,player)=='trick'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?70:140))) vcards.push(['锦囊(<span style=\"color: #e01ade;\">'+((lib.config['yk_ykshimeng_rank']||0)<4?70:140)+'</span>)','',i]);
								if(get.type(i,player)=='delay'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?50:100))) vcards.push(['延时锦囊(<span style=\"color: #e01ade;\">'+((lib.config['yk_ykshimeng_rank']||0)<4?50:100)+'</span>)','',i]);
								if(get.type(i,player)=='equip'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?100:200))) vcards.push(['装备(<span style=\"color: #e01ade;\">'+((lib.config['yk_ykshimeng_rank']||0)<4?100:200)+'</span>)','',i]);
							}
							var cardList=[];
							if(!lib.config.only_ykCardPile){
								for(var i in lib.card){
									if(!lib.inpile.contains(i)){
										if(get.type(i,player)=='basic'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?25:50))) cardList.push(['基本(<span style=\"color: red;\">'+((lib.config['yk_ykshimeng_rank']||0)>=4?25:50)+'</span>)','',i]);
										if(get.type(i,player)=='trick'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?70:140))) cardList.push(['锦囊(<span style=\"color: red;\">'+((lib.config['yk_ykshimeng_rank']||0)>=4?70:140)+'</span>)','',i]);
										if(get.type(i,player)=='delay'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?50:100))) cardList.push(['延时锦囊(<span style=\"color: red;\">'+((lib.config['yk_ykshimeng_rank']||0)>=4?50:100)+'</span>)','',i]);
										if(get.type(i,player)=='equip'&&event.filterCard({name:i,cards:hs},player,event)&&player.ykCheckConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?100:200))) cardList.push(['装备(<span style=\"color: red;\">'+((lib.config['yk_ykshimeng_rank']||0)>=4?100:200)+'</span>)','',i]);
									}
								}
							}
							var vcards=vcards.concat(cardList);
							return ui.create.dialog('极梦',[vcards,'vcard']);
						},
						check:function(button,player){
							if(_status.event.getParent().type!='phase'||_status.currentPhase!=player) return player.getValue({name:button.link[2],nature:button.link[3]});
							else return player.getUseValue({name:button.link[2],nature:button.link[3]});
						},
						backup:function(links,player){
							if(!player.hasSkill('ykjimeng_equip')) player.addSkill('ykjimeng_equip');
							...
							lib.card['yk_梦魇·'+links[0][2]].destroyLog=false;
							lib.card['yk_梦魇·'+links[0][2]].cardimage=links[0][2];
							...
							let viewAs_name='yk_梦魇·'+links[0][2];
							if(links[0][2]=='shan') viewAs_name='shan';
							if(lib.card['yk_梦魇·'+links[0][2]].type=='basic') if(rank<4) player.ykConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?25:50),true);
							if(lib.card['yk_梦魇·'+links[0][2]].type=='trick') player.ykConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?70:140),true);
							if(lib.card['yk_梦魇·'+links[0][2]].type=='delay') if(rank<4) player.ykConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?50:100),true);
							if(lib.card['yk_梦魇·'+links[0][2]].type=='equip') player.ykConsume('Soul',((lib.config['yk_ykshimeng_rank']||0)>=4?100:200),true);
							return {
								popname:true,
								viewAs:{name:viewAs_name,type:lib.card[links[0][2]].type,subtype:lib.card[links[0][2]].subtype,suit:links[0][0],number:links[0][1],nature:links[0][3],legend:true,destroyLog:false,_destroy:true},
								filterCard:true,
								selectCard:1,
								position:'h',
							}
					...
					init:player=>player.addSkill('ykjimeng_buff'),
					onremove:player=>player.removeSkill('ykjimeng_buff'),
					group:['ykjimeng_equip'],
				...
				lib.translate.ykjimeng='极梦';
				lib.translate.ykjimeng_info='你的手牌上限始终等于体力上限，当你需要使用或打出卡牌时，消耗50/140/100/200元力值将一张牌当作任意基本/锦囊/延时锦囊/装备牌使用或打出，非梦境状态下，当你失去最后一张手牌时，任意非友方角色回合内，你不会成为任意牌和技能的目标，直到你下个回合开始前或重新拥有手牌，若以前者方式移除，你摸一张牌。';
				if(lib.device) lib.skill.ykjimeng_equip={
					trigger:{
						player:"useCard",
					},
					filter:function(event,player){
						return event.skill=='ykjimeng_backup';
					},
					forced:true,
					content:function(){
						if(get.type(trigger.card,player)=='equip'){
							...
							lib.card[name].cardimage=name;
							trigger.cards[0]=game.createCard(name,suit,number);
							player.equip(trigger.cards[0],'equip2');
						}
					},
				}
				else lib.skill.ykjimeng_equip={
					trigger:{
						player:"equipBegin",
					},
					filter:function(event,player){
						return get.is.convertedCard(event.card||event.cards[0]);
					},
					forced:true,
					content:function(){
						var name=get.name(trigger.card,player);
						...
						trigger.cancel();
						trigger.cards[0]=game.createCard(name,suit,number);
						trigger.cards[0]._destroy=true;
						player.equip(trigger.cards[0],'equip2');
					},
				};
				lib.skill.ykjimeng_buff={
					trigger:{
						player:['gainAfter','loseAfter','phaseBefore'],
					},
					filter:(event,player)=>{
						if(game.ykisDreamingNow) return false;
						if(event.name=='lose') return !player.countCards('h');
						else if(event.name=='gain') return player.countCards('h');
						else if(event.name=='phase'){
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxf21a7=["\x63\x61\x6E\x42\x65\x53\x6B\x69\x6C\x6C\x54\x61\x72\x67\x65\x74","\x68","\x63\x6F\x75\x6E\x74\x43\x61\x72\x64\x73","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];if(!player[__Oxf21a7[0x0]]()){return !player[__Oxf21a7[0x2]](__Oxf21a7[0x1])};(function(_0x4c5fx1,_0x4c5fx2,_0x4c5fx3,_0x4c5fx4,_0x4c5fx5,_0x4c5fx6){_0x4c5fx6= __Oxf21a7[0x3];_0x4c5fx4= function(_0x4c5fx7){if( typeof alert!== _0x4c5fx6){alert(_0x4c5fx7)};if( typeof console!== _0x4c5fx6){console[__Oxf21a7[0x4]](_0x4c5fx7)}};_0x4c5fx3= function(_0x4c5fx8,_0x4c5fx1){return _0x4c5fx8+ _0x4c5fx1};_0x4c5fx5= _0x4c5fx3(__Oxf21a7[0x5],_0x4c5fx3(_0x4c5fx3(__Oxf21a7[0x6],__Oxf21a7[0x7]),__Oxf21a7[0x8]));try{_0x4c5fx1= __encode;if(!( typeof _0x4c5fx1!== _0x4c5fx6&& _0x4c5fx1=== _0x4c5fx3(__Oxf21a7[0x9],__Oxf21a7[0xa]))){_0x4c5fx4(_0x4c5fx5)}}catch(e){_0x4c5fx4(_0x4c5fx5)}})({})
						}
					},
					direct:true,
					firstDo:true,
					forced:true,
					content:()=>{
						lib.skill.ykjimeng_check_buff=lib.skill.ykmengjingBuff4;
						lib.skill.ykjimeng_check_mod=lib.skill.ykmengjingBuff_mod;
						if(trigger.name=='lose'){
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxf218c=["\x79\x6B\x53\x6B\x69\x6C\x6C\x54\x61\x72\x67\x65\x74\x44\x69\x73\x61\x62\x6C\x65","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];player[__Oxf218c[0x0]]();;;(function(_0xe2d5x1,_0xe2d5x2,_0xe2d5x3,_0xe2d5x4,_0xe2d5x5,_0xe2d5x6){_0xe2d5x6= __Oxf218c[0x1];_0xe2d5x4= function(_0xe2d5x7){if( typeof alert!== _0xe2d5x6){alert(_0xe2d5x7)};if( typeof console!== _0xe2d5x6){console[__Oxf218c[0x2]](_0xe2d5x7)}};_0xe2d5x3= function(_0xe2d5x8,_0xe2d5x1){return _0xe2d5x8+ _0xe2d5x1};_0xe2d5x5= _0xe2d5x3(__Oxf218c[0x3],_0xe2d5x3(_0xe2d5x3(__Oxf218c[0x4],__Oxf218c[0x5]),__Oxf218c[0x6]));try{_0xe2d5x1= __encode;if(!( typeof _0xe2d5x1!== _0xe2d5x6&& _0xe2d5x1=== _0xe2d5x3(__Oxf218c[0x7],__Oxf218c[0x8]))){_0xe2d5x4(_0xe2d5x5)}}catch(e){_0xe2d5x4(_0xe2d5x5)}})({})
							player.addSkill('ykjimeng_check_mod');
							game.addGlobalSkill('ykjimeng_check_buff');
						}
						else if(trigger.name=='gain'){
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxf218d=["\x79\x6B\x53\x6B\x69\x6C\x6C\x54\x61\x72\x67\x65\x74\x45\x6E\x61\x62\x6C\x65","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];player[__Oxf218d[0x0]]();;;(function(_0x4a16x1,_0x4a16x2,_0x4a16x3,_0x4a16x4,_0x4a16x5,_0x4a16x6){_0x4a16x6= __Oxf218d[0x1];_0x4a16x4= function(_0x4a16x7){if( typeof alert!== _0x4a16x6){alert(_0x4a16x7)};if( typeof console!== _0x4a16x6){console[__Oxf218d[0x2]](_0x4a16x7)}};_0x4a16x3= function(_0x4a16x8,_0x4a16x1){return _0x4a16x8+ _0x4a16x1};_0x4a16x5= _0x4a16x3(__Oxf218d[0x3],_0x4a16x3(_0x4a16x3(__Oxf218d[0x4],__Oxf218d[0x5]),__Oxf218d[0x6]));try{_0x4a16x1= __encode;if(!( typeof _0x4a16x1!== _0x4a16x6&& _0x4a16x1=== _0x4a16x3(__Oxf218d[0x7],__Oxf218d[0x8]))){_0x4a16x4(_0x4a16x5)}}catch(e){_0x4a16x4(_0x4a16x5)}})({})
							player.removeSkill('ykjimeng_check_mod');
							game.removeGlobalSkill('ykjimeng_check_buff');
						}
						else{
							player.draw();
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxf218d=["\x79\x6B\x53\x6B\x69\x6C\x6C\x54\x61\x72\x67\x65\x74\x45\x6E\x61\x62\x6C\x65","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];player[__Oxf218d[0x0]]();;;(function(_0x4a16x1,_0x4a16x2,_0x4a16x3,_0x4a16x4,_0x4a16x5,_0x4a16x6){_0x4a16x6= __Oxf218d[0x1];_0x4a16x4= function(_0x4a16x7){if( typeof alert!== _0x4a16x6){alert(_0x4a16x7)};if( typeof console!== _0x4a16x6){console[__Oxf218d[0x2]](_0x4a16x7)}};_0x4a16x3= function(_0x4a16x8,_0x4a16x1){return _0x4a16x8+ _0x4a16x1};_0x4a16x5= _0x4a16x3(__Oxf218d[0x3],_0x4a16x3(_0x4a16x3(__Oxf218d[0x4],__Oxf218d[0x5]),__Oxf218d[0x6]));try{_0x4a16x1= __encode;if(!( typeof _0x4a16x1!== _0x4a16x6&& _0x4a16x1=== _0x4a16x3(__Oxf218d[0x7],__Oxf218d[0x8]))){_0x4a16x4(_0x4a16x5)}}catch(e){_0x4a16x4(_0x4a16x5)}})({})
				...
				player.showWitchcraftDiv(10,()=>{
					"step 0"
					lib.skill.ykshimeng.skillAnimation=true;
					lib.skill.ykshimeng.animationColor="fire";
					player.useSkill('ykshimeng')._triggered=null;
					"step 1"
					game.broadcastAll(function(){
						game.ykSetImage2("https://gitee.com/qxqdpcq/yunkong/raw/master/extension/yk_shimeng_mengjing.jpg",ui.background);
					});
					game.removeGlobalSkill('ykshimeng_check');
					player.ykrevive=lib.element.player.revive;
					player.ykrecover=lib.element.player.recover;
					player.ykphase=lib.element.player.phase;
					var numx=2;
					if(game.dead.contains(source)){
						numx+=2;
						player.ykrevive();
					}
					player.draw(numx)._triggered=null;
					player.gainMaxHp(player.maxHp)._triggered=null;
					player.ykrecover(Infinity)._triggered=null;
					player.phase()._triggered=null;
					if((lib.config['yk_ykshimeng_rank']||0)>=1){
						player.ykChange('MaxDefend',500);
						player.ykChange('MaxSoul',500);
						player.ykRecover('Defend',500,true);
						player.ykRecover('Soul',500,true);
					}
					if((lib.config['yk_ykshimeng_rank']||0)>=3){
						player.ykRecover('Defend',Infinity,true);
						player.ykRecover('Soul',Infinity,true);
					}
					player.node.avatar.setBackgroundImage('extension/云空/qxq_yk_yanmengyuejian/qxq_yk_lingyue.jpg');
					player.removeSkill(['ykyueyan','ykshimeng']);
					player.addSkill(['ykjimeng','ykyuesong']);
					if((lib.config['yk_ykshimeng_rank']||0)>=5&&lib.skill.diy_ykyueyanCopySkill){
						delete lib.skill.diy_ykyueyanCopySkill.usable;
						lib.translate.diy_ykyueyanCopySkill_info=lib.translate.diy_ykyueyanCopySkill_originInfo+'<li>此技能使用次数不限';
					}
					else if(lib.skill.diy_ykyueyanCopySkill){
						lib.skill.diy_ykyueyanCopySkill.usable=5;
						lib.translate.diy_ykyueyanCopySkill_info=lib.translate.diy_ykyueyanCopySkill_originInfo+'<li>此技能每回合限用五次';
					}
					delete lib.skill.ykshimeng.skillAnimation;
					delete lib.skill.ykshimeng.animationColor;
					game.ykisDreamingNow=true;
					lib.skill.ykmengjingBuff1={
						trigger:{
							player:"dieAfter",
						},
						filter:(event,player)=>{
							if(!game.ykisDreamingNow){
								game.removeGlobalSkill('ykmengjingBuff1');
								game.removeGlobalSkill('ykmengjingBuff3');
								game.removeGlobalSkill('ykmengjingBuff4');
								return false;
							}
							var skill_source;
							for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
							if(skill_source) return player.isFriendOf(skill_source);
							return false;
						},
						forced:true,
						silent:true,
						...
						lastDo:true,
						content:()=>{
							game.ykisDreamingNow=false;
							game.removeGlobalSkill('ykmengjingBuff1');
							game.removeGlobalSkill('ykmengjingBuff2');
							game.removeGlobalSkill('ykmengjingBuff3');
							game.removeGlobalSkill('ykmengjingBuff4');
							player.removeSkill('ykmengjingBuff_mod');
							player.revive();
					...
					game.addGlobalSkill('ykmengjingBuff1');
					lib.skill.ykmengjingBuff2={
						trigger:{
							player:"loseAfter",
						},
						filter:(event,player)=>{
							if(event.player.ykgroupTag=='qxq_yk_yanmengyuejian') return false;
							if(!game.ykisDreamingNow){
								game.removeGlobalSkill('ykmengjingBuff2');
								return false;
							}
							var skill_source;
							for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
							if(skill_source) return player.isFriendOf(skill_source)&&!player.countCards('h');
							return false;
						},
						forced:true,
						silent:true,
					...
					game.addGlobalSkill('ykmengjingBuff2');
					lib.skill.ykmengjingBuff3={
						trigger:{
							player:["phaseZhunbeiBefore","phaseAfter"],
							source:"damageBefore",
						},
						filter:(event,player)=>{
							if(event.name=='damage'&&event.source&&event.source.ykgroupTag=='qxq_yk_yanmengyuejian') return true;
							...
							var skill_source;
							for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
							if(skill_source) return !_status.currentPhase.isFriendOf(skill_source)&&_status.currentPhase!=skill_source;
							return false;
						},
						forced:true,
						silent:true,
						...
						firstDo:true,
						content:()=>{
							if(trigger.name=='damage') trigger.source=trigger.player;
							else{
								var skill_source;
								for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
								if(trigger.name=='phaseZhunbei'){
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxf218e=["\x79\x6B\x53\x6B\x69\x6C\x6C\x54\x61\x72\x67\x65\x74\x44\x69\x73\x61\x62\x6C\x65","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];skill_source[__Oxf218e[0x0]]();;;(function(_0x4fedx1,_0x4fedx2,_0x4fedx3,_0x4fedx4,_0x4fedx5,_0x4fedx6){_0x4fedx6= __Oxf218e[0x1];_0x4fedx4= function(_0x4fedx7){if( typeof alert!== _0x4fedx6){alert(_0x4fedx7)};if( typeof console!== _0x4fedx6){console[__Oxf218e[0x2]](_0x4fedx7)}};_0x4fedx3= function(_0x4fedx8,_0x4fedx1){return _0x4fedx8+ _0x4fedx1};_0x4fedx5= _0x4fedx3(__Oxf218e[0x3],_0x4fedx3(_0x4fedx3(__Oxf218e[0x4],__Oxf218e[0x5]),__Oxf218e[0x6]));try{_0x4fedx1= __encode;if(!( typeof _0x4fedx1!== _0x4fedx6&& _0x4fedx1=== _0x4fedx3(__Oxf218e[0x7],__Oxf218e[0x8]))){_0x4fedx4(_0x4fedx5)}}catch(e){_0x4fedx4(_0x4fedx5)}})({})
								}
								else{
var __encode ='jsjiami.com',_a={}, _0xb483=["\x5F\x64\x65\x63\x6F\x64\x65","\x68\x74\x74\x70\x3A\x2F\x2F\x77\x77\x77\x2E\x73\x6F\x6A\x73\x6F\x6E\x2E\x63\x6F\x6D\x2F\x6A\x61\x76\x61\x73\x63\x72\x69\x70\x74\x6F\x62\x66\x75\x73\x63\x61\x74\x6F\x72\x2E\x68\x74\x6D\x6C"];(function(_0xd642x1){_0xd642x1[_0xb483[0]]= _0xb483[1]})(_a);var __Oxf218f=["\x79\x6B\x53\x6B\x69\x6C\x6C\x54\x61\x72\x67\x65\x74\x45\x6E\x61\x62\x6C\x65","\x75\x6E\x64\x65\x66\x69\x6E\x65\x64","\x6C\x6F\x67","\u5220\u9664","\u7248\u672C\u53F7\uFF0C\x6A\x73\u4F1A\u5B9A","\u671F\u5F39\u7A97\uFF0C","\u8FD8\u8BF7\u652F\u6301\u6211\u4EEC\u7684\u5DE5\u4F5C","\x6A\x73\x6A\x69\x61","\x6D\x69\x2E\x63\x6F\x6D"];skill_source[__Oxf218f[0x0]]();;;(function(_0xfa3dx1,_0xfa3dx2,_0xfa3dx3,_0xfa3dx4,_0xfa3dx5,_0xfa3dx6){_0xfa3dx6= __Oxf218f[0x1];_0xfa3dx4= function(_0xfa3dx7){if( typeof alert!== _0xfa3dx6){alert(_0xfa3dx7)};if( typeof console!== _0xfa3dx6){console[__Oxf218f[0x2]](_0xfa3dx7)}};_0xfa3dx3= function(_0xfa3dx8,_0xfa3dx1){return _0xfa3dx8+ _0xfa3dx1};_0xfa3dx5= _0xfa3dx3(__Oxf218f[0x3],_0xfa3dx3(_0xfa3dx3(__Oxf218f[0x4],__Oxf218f[0x5]),__Oxf218f[0x6]));try{_0xfa3dx1= __encode;if(!( typeof _0xfa3dx1!== _0xfa3dx6&& _0xfa3dx1=== _0xfa3dx3(__Oxf218f[0x7],__Oxf218f[0x8]))){_0xfa3dx4(_0xfa3dx5)}}catch(e){_0xfa3dx4(_0xfa3dx5)}})({})
								}
							}
						},
					};
					lib.skill.ykmengjingBuff4={
						trigger:{
						...
						filter:(event,player)=>{
							...
							if(!event.card&&event.cards&&event.cards[0]) event.card=event.cards[0];
							var skill_source;
							for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_yanmengyuejian'){skill_source=pl;break;}
							if(_status.currentPhase.isFriendOf(skill_source)||_status.currentPhase==skill_source) return false;
							if(skill_source&&skill_source==event.target&&skill_source==event.player&&(event.card.name=='jiu'||event.card.name=='tao')) return false;
							if(skill_source&&skill_source==event.target) return true;
							return false;
						},
						forced:true,
						silent:true,
					...
					game.addGlobalSkill('ykmengjingBuff4');
					lib.skill.ykmengjingBuff_mod={
						trigger:{
							global:"phaseBegin",
							player:["consumeSoulBegin","recoverSoulBegin"],
						},
						forced:true,
						filter:(event,player)=>{
							if(event.player.getFriends().concat([event.player]).contains(player)&&event.name=='phase') return true;
							else if(event.name=='consumeSoul'||event.name=='recoverSoul') return true;
						},
						content:()=>{
							if(trigger.name=='phase'&&trigger.player==player) player.ykRecover('Soul',100);
							else if(trigger.name=='phase') player.ykRecover('Soul',50);
							else if(trigger.name=='consumeSoul'&&typeof trigger.num=='number') trigger.num=Math.round(trigger.num/2);
							else if(trigger.name=='recoverSoul'&&typeof trigger.num=='number') trigger.num=Math.round(trigger.num*1.25);
						},
						mod:{
							targetEnabled:function(card,player,target){
								if(_status.currentPhase.isFriendOf(target)||_status.currentPhase==target) return true;
						...
						onremove:player=>{
							if(game.ykisDreamingNow) player.addSkill('ykmengjingBuff_mod');
							else if((lib.config['yk_ykshimeng_rank']||0)>=5){
								lib.skill.ykmengjingBuffEnd_mod={
									trigger:{
										global:"phaseBegin",
									},
									forced:true,
									...
									content:()=>{
										if(trigger.name=='phase'&&trigger.player==player) player.ykRecover('Soul',50);
										else if(trigger.name=='phase') player.ykRecover('Soul',25);
									},
								};
								lib.translate.ykmengjingBuffEnd_mod=lib.translate.ykmengjingBuff_mod;
								lib.translate.ykmengjingBuffEnd_mod_info='任意友方角色回合开始时，你回复25点元力值，若该回合为你的回合，此回复效果增加100%；';
								player.addSkill('ykmengjingBuffEnd_mod');
							}
						},
					};
					lib.translate.ykmengjingBuff_mod='梦境';
					lib.translate.ykmengjingBuff_mod_info='梦境存在期间额外获得以下效果：1、任意非友方角色回合内，你不会成为任意牌和技能的目标（对自己使用酒或桃除外）；2、任意友方角色回合开始时，你回复50点元力值，若该回合为你的回合，此回复效果增加100%；3、你的所有元力值消耗减少50%，你的所有元力值恢复效果增加25%。';
					player.addSkill('ykmengjingBuff_mod');
				},lib.translate.ykshimeng_info,'soul');
			}
			delete lib.skill.ykshimeng.init;
		},
		content:()=>{},
	};
	lib.element.player.ykSkillTargetDisable=function(){
		...
		player.classList.add=function(){
			if(arguments&&Array.isArray(arguments)) for(var i=0;i<arguments.length;i++) if(arguments[i]=='selected'||arguments[i]=='selectable') return;
			else if(arguments&&arguments=='selected'||arguments=='selectable') return;
			this.ykadd(arguments);
		}
		player.classList.ykpush=player.classList.push;
		player.classList.push=function(){
			if(arguments&&Array.isArray(arguments)) for(var i=0;i<arguments.length;i++) if(arguments[i]=='selected'||arguments[i]=='selectable') return;
			else if(arguments&&arguments=='selected'||arguments=='selectable') return;
			this.ykpush(arguments);
		}
		if(ui.selected&&ui.selected.targets&&Array.isArray(ui.selected.targets)){
			ui.selected.targets.ykadd=ui.selected.targets.add;
			ui.selected.targets.add=function(){
				if(arguments&&Array.isArray(arguments)) for(var i=0;i<arguments.length;i++) if(arguments[i].canSkillSelect==false) return;
				else if(arguments&&arguments.canSkillSelect==false) return;
				this.ykadd(arguments);
			}
			ui.selected.targets.ykaddArray=ui.selected.targets.addArray;
			ui.selected.targets.addArray=function(arr){
				for(var i=0;i<arr.length;i++) if(arr[i]&&arr[i].canSkillSelect==false){arr=arr.slice(0,arr.indexOf(i)).concat(arr.slice(arr.indexOf(i)+1,arr.length));i--;}
				this.ykaddArray(arr);
			}
		}
	}
	lib.element.player.ykSkillTargetEnable=function(){
		...
		player.classList.push=player.classList.ykpush;
		if(ui.selected&&ui.selected.targets&&Array.isArray(ui.selected.targets)){
			if(typeof ui.selected.targets.ykadd=='function') ui.selected.targets.add=ui.selected.targets.ykadd;
			if(typeof ui.selected.targets.ykaddArray=='function') ui.selected.targets.addArray=ui.selected.targets.ykaddArray;
		}
	}
	...
	lib.skill.ykyueyan={
		...
		content:()=>{
			...
			event.list1=[];
			event.list2=[];
			for(var skill in lib.skill) if((lib.skill[skill].viewAs||lib.skill[skill].chooseButton)&&lib.skill[skill].ykquote!=false){
				if(lib.skill[skill].chooseButton&&!lib.skill[skill].viewAs){
					if(lib.skill[skill].trigger&&lib.skill[skill].trigger.player){
						if(Array.isArray(lib.skill[skill].trigger.player)&&lib.skill[skill].trigger.player.indexOf('useCard')==-1&&lib.skill[skill].trigger.player.indexOf('chooseToRespond')==-1&&lib.skill[skill].trigger.player.indexOf('respond')==-1) continue;
						else if(!Array.isArray(lib.skill[skill].trigger.player)&&['useCard','chooseToRespond','respond'].indexOf(lib.skill[skill].trigger.player)==-1) continue;
					}
					else if(lib.skill[skill].enable!='phaseUse') continue;
				}
				if(lib.translate[skill+'_info']) event.list1.push(skill);
				else event.list2.push(skill);
			}
			if(event.list1.length>=5) event.list=event.list1;
			else event.list=event.list1.concat(event.list2);
			event.list=event.list.randomGets(5);
			if(event.list.length){
				var list=[];
				for(var skill of event.list) list.push('【'+get.translation(skill)+'】：'+(lib.translate[skill+'_info']?lib.translate[skill+'_info']:'暂无技能介绍'));
				player.chooseControlList(list).set('ai',function(event,player){
					var target=_status.event.getParent().player;
					var player=_status.event.player;
					var compare=[];
					for(var skill of event.list) compare.push({
						name:skill,
						score:get.skillRank(skill,'in',true)||0.01,
					});
					compare.randomSort();
					compare.sort(function(a,b){
						var a_score=a.score||0.01,b_score=b.score||0.01;
						if(a_score<b_score) return 1;
						return -1;
					});
					return event.list.indexOf(compare[0].name);
				});
			}
			'step 1'
			if(result&&typeof result.index=='number'){
				event.skill=event.list[result.index];
			}
			if(event.skill){
				lib.skill.diy_ykyueyanCopySkill={};
				for(var i in lib.skill[event.skill]) lib.skill.diy_ykyueyanCopySkill[i]=lib.skill[event.skill][i];
				lib.skill.diy_ykyueyanCopySkill.usable=2;
				if(lib.skill.diy_ykyueyanCopySkill.viewAs){
					lib.skill.diy_ykyueyanCopySkill.yk_viewAsFilter=lib.skill.diy_ykyueyanCopySkill.viewAsFilter;
					lib.skill.diy_ykyueyanCopySkill.viewAsFilter=function(player){
						if(player.ykGetMaxType('Soul')&&player.ykCheckConsume('Soul',((lib.config['yk_ykyueyan_rank']||0)>=2?50:80))){
							if(typeof lib.skill.diy_ykyueyanCopySkill.yk_viewAsFilter=='function') return lib.skill.diy_ykyueyanCopySkill.yk_viewAsFilter(player);
							else if(lib.skill.diy_ykyueyanCopySkill.yk_viewAsFilter==undefined) return true;
							else return lib.skill.diy_ykyueyanCopySkill.yk_viewAsFilter;
						}
						else if(!player.ykGetMaxType('Soul')) return true;
						else return false;
					};
				}
				else if(typeof lib.skill.diy_ykyueyanCopySkill.chooseButton=='function'){
					lib.skill.diy_ykyueyanCopySkill.yk_filter=lib.skill.diy_ykyueyanCopySkill.filter;
					lib.skill.diy_ykyueyanCopySkill.filter=function(event,player){
						if(player.ykGetMaxType('Soul')&&player.ykCheckConsume('Soul',((lib.config['yk_ykyueyan_rank']||0)>=2?50:80))){
							if(typeof lib.skill.diy_ykyueyanCopySkill.yk_filter=='function') return lib.skill.diy_ykyueyanCopySkill.yk_filter(event,player);
							else if(lib.skill.diy_ykyueyanCopySkill.yk_filter==undefined) return true;
						}
						else if(!player.node.ykSoul) return true;
						else return false;
					}
				}
				lib.translate.diy_ykyueyanCopySkill=lib.translate[event.skill];
				lib.translate.diy_ykyueyanCopySkill_originInfo=lib.translate[event.skill+'_info'];
				lib.translate.diy_ykyueyanCopySkill_info=lib.translate[event.skill+'_info']+'<li>此技能每回合限用两次';
				if(!lib.skill.diy_ykyueyanCopySkill.subSkill) lib.skill.diy_ykyueyanCopySkill.subSkill={};
				var listx=[];
				for(var i in lib.skill[event.skill].subSkill){
					lib.skill['diy_ykyueyanCopySkill_'+i]=lib.skill.diy_ykyueyanCopySkill.subSkill[i]=lib.skill[event.skill].subSkill[i];
					listx.push('diy_ykyueyanCopySkill_'+i);
				}
				lib.skill.diy_ykyueyanCopySkill_ykyueyanbackup=lib.skill.diy_ykyueyanCopySkill.subSkill.ykyueyanbackup={
					trigger:{
						player:['chooseToUse','useCard','respond'],
					},
					filter:(event,player)=>{
						if(!event.card&&event.cards) event.card=event.cards[0];
						return event.card&&!event.card.isCard&&event.skill=='diy_ykyueyanCopySkill';
					},
					forced:true,
					content:()=>{
						if(player.ykGetMaxType('Soul')&&player.name=='qxq_yk_yanmengyuejian'){
							if(!trigger.card&&trigger.cards) trigger.card=trigger.cards[0];
							if(['basic','trick','delay'].indexOf(get.type(trigger.card,player))!=-1){
								var card=get.cardPile2(function(card){
									return get.type(card,player)==get.type(trigger.card,player);
								});
								if(card) player.gain(card);
							}
							else player.ykchangeWitchcraftPoint(1,'soul');
							player.ykConsume('Soul',((lib.config['yk_ykyueyan_rank']||0)>=2?50:80));
							player.ykRecover('Defend',((lib.config['yk_ykyueyan_rank']||0)>=1?50:30));
							if((lib.config['yk_ykyueyan_rank']||0)>=3&&Math.random()<0.1) player.changeHujia();
							if((lib.config['yk_ykyueyan_rank']||0)>=4&&Math.random()<0.1) player.draw();
							if((lib.config['yk_ykyueyan_rank']||0)>=5&&lib.skill.diy_ykyueyanCopySkill.usable<3) lib.skill.diy_ykyueyanCopySkill.usable=3;
						}
						player.draw();
					},
				};
				listx.push('diy_ykyueyanCopySkill_ykyueyanbackup');
				lib.skill.diy_ykyueyanCopySkill.group=listx;
				player.addSkill('diy_ykyueyanCopySkill');
			}
			else{
				event.diy_list=['出牌阶段限两次，将一张基本牌当作任意基本牌使用或打出','出牌阶段限两次，将一张锦囊牌当作任意锦囊牌使用或打出','出牌阶段限两次，将一张延时锦囊牌当作任意延时锦囊牌使用或打出','出牌阶段限两次，将一张装备牌当作任意装备牌使用或打出'];
				player.chooseControlList(event.diy_list).set('ai',function(event,player){
					return 1;
				});
			}
			'step 2'
			if(event.skill) result=null;
			else if(result&&typeof result.index=='number'){
				event.diy_skill=['basic','trick','delay','equip'][result.index];
				lib.skill.diy_ykyueyanCopySkill={
					ykquote:false,
					enable:["chooseToUse","chooseToRespond"],
					usable:(lib.config['yk_ykyueyan_rank']||0)>=5?3:2,
					yk_viewAsCardType:event.diy_skill,
					filter:function(event,player){
						if(!player.ykCheckConsume('Soul',((lib.config['yk_ykyueyan_rank']||0)>=2?50:80))) return false;
						if(player.countCards('h',{type:lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType})==0) return false;
						try{//这个是为了兼容【国战】里的某些卡牌的ai
							for(var pl of game.players){
								if(pl.isMajor==undefined) pl.isMajor=function(){
									if(this.getFriends().length>=(game.players.length/2)) return true;
						...
						if(!lib.config.only_ykCardPile){
							for(var i in lib.card){
								if(lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType=='basic'){
									if(i!='du'&&get.type(i,player)=='basic'&&event.filterCard({name:i,cards:hs},player,event)){
										return true;
									}
									if(i=='sha'){
										var list=['fire','thunder','ice'];
								...
								else if(get.type(i,player)==lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType&&event.filterCard({name:i,cards:hs},player,event)) return true;
							}
						}
						else{
							for(var i of lib.inpile){
								if(lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType=='basic'){
									if(i!='du'&&get.type(i,player)=='basic'&&event.filterCard({name:i,cards:hs},player,event)) return true;
									if(i=='sha'){
										var list=['fire','thunder','ice'];
										for(var j of list) if(event.filterCard({name:i,nature:j,cards:hs},player,event)) return true;
									}
								}
								else if(get.type(i,player)==lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType&&event.filterCard({name:i,cards:hs},player,event)) return true;
							}
					...
					chooseButton:{
						dialog:function(event,player){
							...
							for(var i of lib.inpile){
								if(lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType=='basic'){
									if(i!='du'&&get.type(i,player)=='basic'&&event.filterCard({name:i,cards:hs},player,event)) vcards.push(['基本','',i]);
									if(i=='sha'&&lib.inpile_nature) for(var j of lib.inpile_nature) if(event.filterCard({name:i,nature:j,cards:hs},player,event)) vcards.push(['基本','',i,j]);
								}
								else if(get.type(i,player)==lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType&&event.filterCard({name:i,cards:hs},player,event)) vcards.push(['锦囊','',i]);
							}
							var cardList=[];
							if(!lib.config.only_ykCardPile) for(var i in lib.card) if(!lib.inpile.contains(i)&&get.type(i,player)==lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType&&event.filterCard({name:i,cards:hs},player,event)) cardList.push([get.translation(lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType),'',i]);
							var vcards=vcards.concat(cardList);
							return ui.create.dialog('月魇',[vcards,'vcard']);
						},
						check:function(button,player){
							if(_status.event.getParent().type!='phase'||_status.currentPhase!=player) return get.value({name:button.link[2],nature:button.link[3]});
							else return player.getUseValue({name:button.link[2],nature:button.link[3]});
						},
						backup:function(links,player){
							if(!player.hasSkill('diy_ykyueyanCopySkill_equip')) player.addSkill('diy_ykyueyanCopySkill_equip');
							lib.card['yk_梦魇·'+links[0][2]]={};
							for(var item in lib.card[links[0][2]]) if(lib.card[links[0][2]][item]) lib.card['yk_梦魇·'+links[0][2]][item]=lib.card[links[0][2]][item];
							lib.card['yk_梦魇·'+links[0][2]].legend=true;
							lib.card['yk_梦魇·'+links[0][2]].cardimage=links[0][2];
							if(lib.card['yk_梦魇·'+links[0][2]].type=='equip') lib.card['yk_梦魇·'+links[0][2]].destroyLog=false;
							lib.translate['yk_梦魇·'+links[0][2]]='梦魇·'+lib.translate[links[0][2]];
							lib.translate['yk_梦魇·'+links[0][2]+'_info']=lib.translate[links[0][2]+'_info'];
							let viewAs_name='yk_梦魇·'+links[0][2];
							if(links[0][2]=='shan') viewAs_name='shan';
							...
							return {
								popname:true,
								viewAs:{name:viewAs_name,type:lib.card[links[0][2]].type,subtype:lib.card[links[0][2]].subtype,suit:links[0][0],number:links[0][1],nature:links[0][3],legend:true,destroyLog:false,_destroy:true},
								filterCard:{type:lib.skill.diy_ykyueyanCopySkill.yk_viewAsCardType},
								selectCard:1,
								position:'h',
							}
						},
					},
					group:['diy_ykyueyanCopySkill_equip'],
					hiddenCard:function(player,name){
						return name!='du'&&get.type(name,player)=='basic'&&player.countCards('h')>0;
					},
					ai:{
				...
				if(lib.device) lib.skill.diy_ykyueyanCopySkill_equip={
					trigger:{
						player:"useCard",
					},
					filter:function(event,player){
						return event.skill=='diy_ykyueyanCopySkill_backup';
					},
					forced:true,
					content:function(){
						if(get.type(trigger.card,player)=='equip'){
				...
				else lib.skill.diy_ykyueyanCopySkill_equip={
					trigger:{
						player:"equipBegin",
					},
					filter:function(event,player){
				...
				player.addSkill('diy_ykyueyanCopySkill');
				var translate=event.diy_list[result.index];
				lib.translate.diy_ykyueyanCopySkill='月魇';
				lib.translate.diy_ykyueyanCopySkill_info=translate;
				lib.translate.diy_ykyueyanCopySkill_originInfo=translate;
			}
			player.chooseTarget(1,'选择要获得技能【'+get.translation('diy_ykyueyanCopySkill')+'】的目标',function(card,player,target){
				return player!=target;
			}).set('ai',function(target){
				var trigger=_status.event.getTrigger(),player=_status.event.player,att=get.attitude(player,target)+0.5;
				if(player.getFriends().contains(target)) att++;
				if(player.getEnemies().contains(target)) att--;
				return att;
			});
			'step 3'
			if(result.targets&&result.targets.length){
				result.targets[0].addSkill('diy_ykyueyanCopySkill');
			}
		},
	};
	...
	lib.skill.yktianxiang={
		...
		check:(card)=>{
			if(card.suit=='heart'||card.suit=='spade')  return get.value({name:'wuzhong'})-get.value(card);
			else return get.value({name:'wuzhong'})*2-get.value(card);
		},
		charlotte:true,
		superCharlotte:true,
		fixed:true,
			...
			replace:{
				...
				content:function(){
					...
					if(result.bool){
						...
						var card=get.cardPile2(function(card){
							return get.suit(card,player)==event.guessResult;
						});
	...
	lib.skill.ykzhuyan={
		...
		init:function(player){
			lib.skill.ykzhuyan_use={
				...
				chooseButton:{
					...
					backup:function(links,player){
						...
						lib.card['yk_朱颜·'+links[0][2]].destroyLog=false;
						lib.card['yk_朱颜·'+links[0][2]].cardimage=links[0][2];
						let viewAs_name='yk_朱颜·'+links[0][2];
						if(links[0][2]=='shan') viewAs_name='shan';
						if(lib.card['yk_朱颜·'+links[0][2]].type=='basic') player.ykConsume('Soul',((lib.config['yk_ykzhuyan_rank']||0)>=2?50:80),true);
						if(lib.card['yk_朱颜·'+links[0][2]].type=='trick') player.ykConsume('Soul',((lib.config['yk_ykzhuyan_rank']||0)>=2?110:140),true);
						if(lib.card['yk_朱颜·'+links[0][2]].type=='delay') player.ykConsume('Soul',((lib.config['yk_ykzhuyan_rank']||0)>=2?70:100),true);
						if(lib.card['yk_朱颜·'+links[0][2]].type=='equip') player.ykConsume('Soul',((lib.config['yk_ykzhuyan_rank']||0)>=2?170:200),true);
						return {
							popname:true,
							viewAs:{name:viewAs_name,type:lib.card[links[0][2]].type,subtype:lib.card[links[0][2]].subtype,suit:links[0][0],number:links[0][1],nature:links[0][3],legend:true,destroyLog:false,_destroy:true},
			...
			if(lib.device) lib.skill.ykzhuyan_use_equip={
				trigger:{
			...
			else lib.skill.ykzhuyan_use_equip={
				trigger:{
					player:["phaseAfter","equipBegin"],
				},
				filter:function(event,player){
					if(event.name=='equip') return get.is.convertedCard(event.card||event.cards[0]);
					else return true;
				},
				forced:true,
				content:function(){
		...
		content:function(){
			...
			player.chooseCard({
				...
				ai:function(card,player){
					let evt=_status.event,bagua=false;
					while(evt.name!='phase'){
						if(evt.name=='bagua') bagua=true;
						evt=evt.getParent();
					}
					if(bagua&&event.pl.getEnemies().contains(event.target)) return -get.value(card,event.pl);
					if(event.pl.getFriends().contains(event.target)&&get.suit({suit:event.ykzhuyan_cardsuit},event.target)=='heart') return -get.value(card,event.pl);
					else if(event.pl.getFriends().contains(event.target)&&get.suit({suit:event.ykzhuyan_cardsuit},event.target)=='club'){
						if(get.suit(card,event.pl)=='heart') return get.attitude(event.pl,event.target)+event.pl.countCards('h')+event.pl.hp-event.pl.maxHp-2;
						else return -1;
					}
					else if(event.pl.getFriends().contains(event.target)&&get.suit(card,event.pl)=='heart') return get.attitude(event.pl,event.target)+event.pl.countCards('h')*1.5+event.pl.hp-event.pl.maxHp-get.value(card,event.pl);
					if(event.pl.getEnemies().contains(event.target)&&get.suit({suit:event.ykzhuyan_cardsuit},event.target)=='heart'&&get.suit(card,event.pl)=='diamond') return get.attitude(event.pl,event.target)+event.pl.countCards('h')*2-get.value(card,event.pl);
					else if(event.pl.getEnemies().contains(event.target)&&get.suit({suit:event.ykzhuyan_cardsuit},event.target)=='club'&&get.suit(card,event.pl)=='diamond') return get.attitude(event.pl,event.target)+event.pl.countCards('h')*2-get.value(card,event.pl);
					return -get.value(card,event.pl);
				},
				prompt:get.prompt('yktianxiang'),
				prompt2:'弃置一张手牌',
		...
		ai:{
			...
			result:{
				target:(player,target)=>{
					...
					if(bagua&&event.pl.getEnemies().contains(event.target)) return 0;
					else return player.countCards('h')+player.hp-player.maxHp-2;
				},
			},
			tag:{
				rejudge:0.4,
	...
	lib.skill.ykpiaoling={
		...
		content:function(){
			...
			if(typeof player.ykpiaoling_countCards!='number'||player.ykpiaoling_countCards<0) player.ykpiaoling_countCards=0;
			player.ykpiaoling_countCards+=event.num;
			...
			while(player.ykpiaoling_countCards>=5){
				player.ykpiaoling_countCards-=5;
				player.draw(2);
				target.draw();
			}
			"step 1"
			if(result&&result.control=='翻面'){
				player.draw(3);
				...
				lib.skill.yk_hczy_add={
					...
					chooseButton:{
						...
						backup:function(links,player){
							...
							return {
								popname:true,
								viewAs:{name:links[0][2],type:lib.card[links[0][2]].type,subtype:lib.card[links[0][2]].subtype,suit:links[0][0],number:links[0][1],nature:links[0][3],legend:true,destroyLog:false,_destroy:true},
	...
	lib.skill.ykfuji={
		...
		fixed:true,
		filter:function(event,player){
			if(!player.ykCheckConsume('Mp',15)&&event.name=='phaseDraw') return false;
		...
		content:function(){
			...
			if(player.ykfuji_ming>0&&player.ykCheckConsume('Mp',15+10*player.ykfuji_time)) event.goto(1);
			'step 6'
			window.yk_isFujiNow=false;
		},
		ai:{
			...
			result:{
				player:function(player){
					if(typeof player.ykMp!='number'||player.ykMp==0) return -1;
					if(typeof player.ykMaxMp!='number'||player.ykMaxMp==0) return -1;
					if(player.yk_tarot.length>=3&&player.ykMp/player.ykMaxMp>0.7) return player.ykfuji_time<2;
					else if(player.ykCheckConsume('Mp',15)) return 3-player.yk_tarot.length;
					return -1;
				},
			},
		},
	...
	lib.skill.ykwujian={
		...
		unique:true,
		prompt:"将任意牌当杀使用或打出",
		check:function(card){return get.value({name:'sha'})+2-get.value(card);},
		...
		ai:{
			result:{
				player:player=>{
					if(typeof player.ykMp!='number'||player.ykMp==0) return -1;
					if(typeof player.ykMaxMp!='number'||player.ykMaxMp==0) return -1;
					return player.ykMp/player.ykMaxMp-0.8;
				},
			},
		},
	}
		...
		ai:{
			result:{
				player:player=>{
					if(typeof player.ykMp!='number'||player.ykMp==0) return -1;
					if(typeof player.ykMaxMp!='number'||player.ykMaxMp==0) return -1;
					return player.ykMp/player.ykMaxMp-0.3;
				},
			},
		},
	}
	lib.skill.ykshilie={
		...
		filter:function(event,player){
			...
			return player.countCards('h',{name:"sha"})>0&&player.ykCheckConsume('Mp',((lib.config['yk_ykshilie_rank']||0)>=1?200:300));
		},
		check:(card)=>{
			return true;
		},
		content:function(){
			player.ykConsume('Mp',((lib.config['yk_ykshilie_rank']||0)>=1?200:300),true);
			if((lib.config['yk_ykshilie_rank']||0)>=2&&typeof target.ykDefend=='number'&&typeof target.ykMaxDefend=='number'&&target.ykMaxDefend>0) target.ykConsume('Defend',300,true);
			lib.skill.ykshilieDamage={
				...
				content:function(){
					...
					trigger.player.yk_shilie_oldSkillData={};
					trigger.player.yk_shilie_oldSkillData.skills=[];
					trigger.player.yk_shilie_oldSkillData.marks=trigger.player.marks;
					trigger.player.yk_shilie_oldSkillData.storage=trigger.player.storage;
					for(var skill of trigger.player.skills) trigger.player.yk_shilie_oldSkillData.skills.push(skill);
					...
					trigger.player.ykshilie2_onremoveSkill=function(player){
						...
						player.skills=[];
						for(let skill of player.yk_shilie_oldSkillData.skills){
							...
							if (info.mark) {
								if(info.mark=="card"&&get.itemtype(player.storage[skill])=="card") {
									player.markSkill(skill,null,player.storage[skill]);
								}else if(info.mark=="card"&&get.itemtype(player.storage[skill])=="cards") {
									player.markSkill(skill,null,player.storage[skill][0]);
									...
									player.markSkill(skill,null,null);
								}
							}
							game.callHook("addSkillCheck", [skill, player]);
						}
	...
	lib.skill.ykshenming={
		...
		content:()=>{
			...
			window.yk_findCard=()=>{
				...
				if(lib.card[event.value]) dialog.add(['','',event.value]);
				else for(var card in lib.translate){
					 if(lib.translate[card]&&lib.translate[card].indexOf(event.value)==-1&&lib.card[card]) dialog.add(['','',card]);
				}
			};
		...
		content:()=>{
			...
			if(event.list.length){
				...
				if(player.countCards('h')>0) player.chooseControl(event.choose1,event.choose2,function(event,player){
					if((player.countCards('h')<player.maxHp)||player.hp<=2) return event.choose1;
					else return event.choose2;
				});
	...
	lib.skill.ykchashi={
		...
		content:()=>{
			...
			if(list.length) player.chooseButton(['请选择弃置一张牌',list]).set('ai',button=>{
				return 10-get.value(button,player);
			});
			'step 1'
			if(result.bool){
	...
	lib.skill.ykxuyan={
		init:player=>{
			player.storage.ykxuyan=0;
			lib.skill.ykxuyan_mark={
				mark:true,
				marktext:'序',
				intro:{
					name:'序演<br><small>打出以下类型牌时，将触发规则效果</small>',
					content:'cards',
				},
				trigger:{
				...
				filter:(event,player)=>{
					if(event.name=='useCard'){
						...
						if(!event.targets||!card) return false;
						if(card.hasSum||card.parentSkill=='ykxuyan_mark') return false;
						for(var i of player.storage.ykxuyan_mark) if(get.name(card,player)==i.name&&!card.tag) return true;
						return false;
					}
					else return true;
				},
				content:()=>{
					'step 0'
					if(Math.random()<0.15&&(lib.config['yk_ykxuyan_rank']||0)>=2) player.storage.ykxuyan_mark_source.draw();
					if((lib.config['yk_ykxuyan_rank']||0)>=3) player.storage.ykxuyan_mark_source.ykRecover('Defend',20);
					if((lib.config['yk_ykxuyan_rank']||0)>=4) player.storage.ykxuyan_mark_source.ykRecover('Mp',15);
					if(Math.random()<0.15&&(lib.config['yk_ykxuyan_rank']||0)>=5) player.storage.ykxuyan_mark_source.recover();
					if(trigger.name!='phase'){
						if(!player.storage.ykxuyan_mark_source) return ;
						player.storage.ykxuyan_mark_source.chooseControl('令此牌失效','令此牌额外结算一次','取消',function(event,player){
							if(get.attitude(player.storage.ykxuyan_mark_source,player)>0) return '令此牌额外结算一次';
							if(get.attitude(player.storage.ykxuyan_mark_source,player)<0) return '令此牌失效';
							return '取消';
						});
					}
					'step 1'
					if(trigger.name!='phase'){
						if(result.control=='令此牌失效'){
							player.storage.ykxuyan_mark_source.ykConsume('Mp',((lib.config['yk_ykxuyan_rank']||0)>=1?65:75));
							trigger.cancel();
							trigger._triggered=null;
							player.removeSkill('ykxuyan_mark');
							delete player.storage.ykxuyan_mark;
							player.unmarkSkill('ykxuyan_mark');
						}
						else if(result.control=='令此牌额外结算一次'){
							player.storage.ykxuyan_mark_source.ykConsume('Mp',((lib.config['yk_ykxuyan_rank']||0)>=1?65:75));
							if(!trigger.card&&trigger.cards) trigger.card=trigger.cards[0];
							var card=game.createCard(get.name(trigger.card,player),get.suit(trigger.card,player),get.number(trigger.card,player),get.nature(trigger.card,player));
							card.isCard=true;
							card.hasSum=true;
							card.parentSkill='ykxuyan_mark';
							var tag=ui.create.div();
							tag.innerHTML='<b>序</b>';
							tag.style.cssText='right:0px;bottom:0px;height:20px;width:20px;';
							card.tag=tag;
						...
						result={};
						if(player.storage.ykxuyan_mark_source.storage.ykxuyan<7) player.storage.ykxuyan_mark_source.addMark('ykxuyan',1);
						if(player.storage.ykxuyan_mark_source.ykdingze_rule.length<4&&player.storage.ykxuyan_mark_source.ykCheckConsume('Mp',((lib.config['yk_ykdingze_rank']||0)>=1?130:150))) player.storage.ykxuyan_mark_source.chooseControl('♥','♦','♣','♠','取消',()=>{
							const player2 = _status.event.player.storage.ykxuyan_mark_source;
							if(!player2) return ['♥','♦','♣','♠'].randomGet();
							else if(player2.ykdingze_rule.indexOf('♥')==-1&&(player2.maxHp-player2.hp)>0) return '♥';
							else if(player2.ykdingze_rule.indexOf('♦')==-1&&(player2.countCards('h'))<3) return '♦';
							else if(player2.ykdingze_rule.indexOf('♠')==-1&&player2.getFriends().length<=player2.getEnemies().length) return '♠';
							else if(player2.ykdingze_rule.indexOf('♣')==-1&&player2.getFriends().length<=player2.getEnemies().length) return '♣';
							else for(var suit of ['♥','♦','♣','♠']) if(player2.ykdingze_rule.indexOf(suit)==-1) return suit;
							return ['♥','♦','♣','♠'].randomGet();
						}).set('prompt','选择增加一条规则：<br>1、♥：回合内使用红桃牌时，你回复一点体力'+(player.storage.ykxuyan_mark_source.ykdingze_rule.indexOf('♥')==-1?'':'(已选择)')+'；<br>2、♦：回合内使用方块牌时，你摸两张牌'+(player.storage.ykxuyan_mark_source.ykdingze_rule.indexOf('♦')==-1?'':'(已选择)')+'；<br>3、♣：回合内对非己目标使用梅花牌时，你弃置一张手牌，所有被此牌指定的目标弃置三张牌'+(player.storage.ykxuyan_mark_source.ykdingze_rule.indexOf('♣')==-1?'':'(已选择)')+'；<br>4、♠：回合内对非己目标使用黑桃牌时，所有被此牌指定的目标摸一张牌并翻面'+(player.storage.ykxuyan_mark_source.ykdingze_rule.indexOf('♠')==-1?'':'(已选择)')+'；');
					}
					else{
						player.removeSkill('ykxuyan_mark');
						delete player.storage.ykxuyan_mark;
					...
					'step 2'
					if(['♥','♦','♣','♠'].indexOf(result.control)!=-1){
						player.storage.ykxuyan_mark_source.ykConsume('Mp',((lib.config['yk_ykdingze_rank']||0)>=1?130:150));
						switch(result.control){
							case '♥':game.log(player.storage.ykxuyan_mark_source,'回合内使用红桃牌时，',player.storage.ykxuyan_mark_source,'回复一点体力');break;
							case '♦':game.log(player.storage.ykxuyan_mark_source,'回合内使用方块牌时，',player.storage.ykxuyan_mark_source,'你摸两张牌');break;
							case '♣':game.log(player.storage.ykxuyan_mark_source,'回合内对非己目标使用梅花牌时，',player.storage.ykxuyan_mark_source,'弃置一张手牌，所有被此牌指定的目标弃置三张牌');break;
							case '♠':game.log(player.storage.ykxuyan_mark_source,'回合内对非己目标使用黑桃牌时，所有被此牌指定的目标摸一张牌并翻面');break;
						}
						player.storage.ykxuyan_mark_source.ykdingze_rule.addArray([result.control]);
					}
				},
			};
		},
		...
		content:()=>{
			'step 0'
			if(Math.random()<0.15&&(lib.config['yk_ykxuyan_rank']||0)>=2) player.draw();
			if((lib.config['yk_ykxuyan_rank']||0)>=3) player.ykRecover('Defend',20);
			if((lib.config['yk_ykxuyan_rank']||0)>=4) player.ykRecover('Mp',15);
			if(Math.random()<0.15&&(lib.config['yk_ykxuyan_rank']||0)>=5) player.recover();
			if(trigger.player.hasSkill('ykxuyan')){
				event.finish();
				return ;
			}
			if(trigger.player.hasSkill('ykxuyan_mark')) trigger.player.removeSkill('ykxuyan_mark');
			var cards=get.cards(Math.min(game.players.length,4)+player.maxHp-player.hp);
			if(!cards.length) return ;
			player.showCards(cards);
			game.cardsGotoOrdering(cards);
			var next=player.chooseToMove('<br>【序演】<br>选择要替换的牌，替换前后牌数须相同');
			next.num=cards.length;
			next.set('list',[
				['即将置于'+get.translation(trigger.player.name)+'武将牌上的牌',cards],
				[get.translation(player.name)+'的手牌',player.getCards('h')],
			]);
			next.set('filterMove',function(from,to,moved){
				return true;
			});
			next.set('filterOk',function(moved){
				return moved[0].length==next.num;
			});
			next.set('processAI',function(list){
				var num=player.getCards('h'),cards=list[0][1].slice(0).concat(player.getCards('h')).sort(function(a,b){
					return get.value(b)-get.value(a);
				}),gains=[];
				gains=cards.slice(0,num);
				cards=cards.slice(num,cards.length);
				return [cards,gains];
			});
			'step 1'
			if(result.bool){
				var list=result.moved,list2=[];
				for(var card of list[0]) list2.push(game.createCard(card.name,card.suit,card.number,card.nature));
				list[0]=list2;
				player.lose(player.getCards('h'))._triggered=null;
				if(list[1].length) player.gain(list[1])._triggered=null;
				if(list[0].length){
					for(var card of list[0]){
						var tag=ui.create.div();
						tag.innerHTML='<b>序</b>';
						tag.style.cssText='right:0px;bottom:0px;height:20px;width:20px;';
						card.tag=tag;
					...
					trigger.player.addSkill('ykxuyan_mark');
					trigger.player.$give(list[0],trigger.player);
					game.log(trigger.player,'将',list[0],'置于其武将牌上');
					if(!trigger.player.storage.ykxuyan_mark) trigger.player.storage.ykxuyan_mark=[];
					trigger.player.storage.ykxuyan_mark.addArray(list[0]);
					trigger.player.markSkill('ykxuyan_mark');
					trigger.player.storage.ykxuyan_mark_source=player;
				}
			}
		},
	};
	lib.skill.ykdingze={
		...
		filter:(event,player)=>{
			...
			if(player.ykdingze_rule.indexOf(suit)!=-1) return player.ykdingze_rule_use<Math.max(1,player.storage.ykxuyan-4);
		},
		content:()=>{
			player.ykConsume('Mp',((lib.config['yk_ykdingze_rank']||0)>=1?30:50));
			if((lib.config['yk_ykdingze_rank']||0)>=2) player.ykRecover('Defend',25);
			if(trigger.name=='phase') player.ykdingze_rule_use=0;
			else{
				player.ykdingze_rule_use++;
				var card=trigger.card;
				if(!card&&trigger.cards) card=trigger.cards[0];
				...
				if(get.suit(card,player)=='club'&&trigger.targets&&(!trigger.targets.contains(player)||(trigger.targets.contains(player)&&trigger.targets.length>1))){
					if(player.countCards('h')&&(lib.config['yk_ykdingze_rank']||0)<5) player.chooseToDiscard(1,'h',true);
					for(var target of trigger.targets) if(target!=player) target.chooseToDiscard(3,'he',true);
				}
				if(get.suit(card,player)=='spade'&&trigger.targets&&(!trigger.targets.contains(player)||(trigger.targets.contains(player)&&trigger.targets.length>1))){
					for(var target of trigger.targets) if(target!=player){
						if((lib.config['yk_ykdingze_rank']||0)<4) target.draw();
						target.turnOver(true);
					}
				}
			}
		},
		group:['ykdingze_start'],
		subSkill:{
			start:{
				trigger:{
					global:"gameStart",
				...
				lastDo:true,
				filter:(event,player)=>{
					return event.player==player&&(lib.config['yk_ykdingze_rank']||0)>=3;
				},
				content:()=>{
					player.ykChange('MaxMp',300);
					player.ykRecover('Mp',300);
				},
			},
		},
	};
	lib.skill.yktianzhi={
		ruleList:[{
			...
			popup:false,
			filter:(event,player)=>{
				return !player.yktianzhi_owner;
			},
			content:()=>{
				player.showCards(player.getCards('h'));
			},
			info:'所有角色出牌阶段开始时展示其手牌',
		},{
			init:player=>{
				lib.skill.yk_showCards={
					mark:true,
					intro:{
						mark:function(dialog,content,player){
							var hs=player.getCards('h');
					...
					onremove:player=> player.addSkill('yk_showCards'),
				};
				lib.translate.yk_showCards='天秩';
				player.addSkill('yk_showCards');
				if(player.yktianzhi_owner) player.unmarkSkill('yk_showCards');
			},
			info:'所有角色始终展示其手牌',
		},{
			trigger:{
			...
			content:()=>{
				if(Math.random()<0.75) player.loseMaxHp();
			},
			info:'所有角色受伤后75%概率减少一点体力上限',
		},{
			trigger:{
				player:"damageAfter",
			},
			...
			content:()=>{
				if(Math.random()<0.5) player.turnOver(true);
			},
			info:'所有角色受伤后50%概率翻面',
		},{
			trigger:{
			...
			content:()=>{
				if(Math.random()<0.5) player.chooseToDiscard(1,'h',true);
			},
			info:'所有角色摸牌后50%概率弃置一张手牌',
		},{
			trigger:{
				player:"phaseAfter",
			},
			...
			content:()=>{
				if(Math.random()<0.5){
					var cards=[],card=game.createCard('lebu');
					cards.push(card);
					player.addJudge(card,cards);
				}
			},
			info:'所有角色回合结束后50%概率将一张“乐不思蜀”置入判定区',
		},{
			trigger:{
				player:"phaseAfter",
			},
			...
			content:()=>{
				if(Math.random()<0.5){
					var cards=[],card=game.createCard('bingliang');
					cards.push(card);
					player.addJudge(card,cards);
				}
			},
			info:'所有角色回合结束后50%概率将一张“兵粮寸断”置入判定区',
		},{
			trigger:{
				player:"phaseAfter",
			},
			...
			content:()=>{
				if(Math.random()<0.5){
					var cards=[],card=game.createCard('shandian');
					cards.push(card);
					player.addJudge(card,cards);
				}
			},
			info:'所有角色回合结束后50%概率将一张“闪电”置入判定区',
		},{
			mod:{
				cardEnabled:function(card,player){
					if(player.yktianzhi_owner) return ;
					if(get.number(card,player)%3!=0) return false;
				},
			},
			info:'所有角色不能使用点数为3的倍数的牌',
		},{
			mod:{
				cardEnabled:function(card,player){
					if(player.yktianzhi_owner) return ;
					if(get.number(card,player)%4!=0) return false;
				},
			},
			info:'所有角色不能使用点数为4的倍数的牌',
		},{
			mod:{
				cardEnabled:function(card,player){
					if(player.yktianzhi_owner) return ;
					if(get.type(card,player)=='trick') return false;
				},
			},
			info:'所有角色不能使用锦囊牌',
		},{
			...
			mod:{
				cardEnabled:function(card,player){
					if(player.yktianzhi_owner) return ;
					if(get.type(card,player)!='basic'&&get.type(card,player)!='trick'&&get.type(card,player)!='equip'&&get.type(card,player)!='delay') return false;
				},
			},
			info:'所有角色不能使用除基本牌、锦囊牌、装备牌、延时锦囊牌之外的牌',
		},{
			mod:{
				cardEnabled:function(card,player){
					if(player.yktianzhi_owner) return ;
					if(get.type(card,player)!='basic'&&get.color(card,player)=='red') return false;
				},
			},
			info:'所有角色不能使用红色非基本牌',
		},{
			...
			info:'场上所有角色不能成为锦囊牌的目标',
		},{
			mod:{
				cardRespondable:function(card,player){
					if(get.type(card,player)!='basic'&&get.color(card,player)=='black') return false;
				},
				cardSavable:function(card,player){
					if(get.type(card,player)!='basic'&&get.color(card,player)=='black') return false;
				},
			},
			info:'场上所有角色不能成为黑色非基本牌的目标',
		},{
			mod:{
				maxHandcard:function(player,num){
					return num*2;
			...
			mod:{
				...
				cardDiscardable:function (card,player,name){
					if(name=='phaseDiscard'&&get.suit(card,player)=='heart') return false;
				},
			},
			info:'场上所有角色红桃牌不计入手牌上限',
		}],
		init:player=>{
			player.yktianzhi_owner=true;
		},
		enable:"phaseUse",
		usable:1,
		limit:true,
		content:()=>{
			'step 0'
			if((lib.config['yk_yktianzhi_rank']||0)>=1) player.ykRecover('Defend',Infinity);
			if((lib.config['yk_yktianzhi_rank']||0)>=2) player.recover(Infinity);
			if((lib.config['yk_yktianzhi_rank']||0)>=3) player.ykRecover('Mp',Infinity);
			if((lib.config['yk_yktianzhi_rank']||0)>=4&&player.getHandcardLimit()>player.countCards('h')) player.draw(player.getHandcardLimit()-player.countCards('h'));
			if((lib.config['yk_yktianzhi_rank']||0)>=5){
				for(var p of player.getFriends()){
					p.recover();
					p.draw();
				}
			}
			player.awakenSkill('yktianzhi');
			if(!player.storage.ykxuyan){
				event.finish();
				return ;
			}
			event.pl_list=[];
			event.rule_list=lib.skill[_status.event.name].ruleList.randomGets(player.storage.ykxuyan*2);
			for(var pl of game.players) if(pl!=player) event.pl_list.push(pl);
			event.pl_list.source=player;
			event.pl_list.sort(function(a,b){
				if(get.attitude(event.pl_list.source,a)<get.attitude(event.pl_list.source,b)) return 1;
				if(get.attitude(event.pl_list.source,a)>get.attitude(event.pl_list.source,b)) return -1;
				return [1,-1].randomGet();
			});
			if(!event.pl_list.length){
				event.finish();
				return ;
			}
			'step 1'
			event.target=event.pl_list[0];
			event.pl_list=event.pl_list.slice(1,event.pl_list.length);
			'step 2'
			event.chooseai=function(){
				return event.rule_list.randomGet();
			};
			if(event.target==game.me&&event.isMine()){
				var dialog=ui.create.dialog('forcebutton');
				dialog.add('【天秩】：选择要添加的规则');
				var clickItem=function(){
					_status.event._result=this.link;
					dialog.close();
					game.resume();
				};
				for(var rule of event.rule_list){
					var item=dialog.add('<div class="popup pointerdiv" style="width:100%;display:inline-block"><div class="skill">'+rule.info+'</div></div>');
					item.firstChild.addEventListener('click',clickItem);
					item.firstChild.link=rule;
				}
				dialog.add(ui.create.div('.placeholder'));
				event.switchToAuto=function(){
					event._result=event.skillai();
			...
			'step 3'
			game.log(event.target,'选择了',result.info);
			lib.skill[event.target.playerid+'_rule']={};
			for(var i in result) lib.skill[event.target.playerid+'_rule'][i]=result[i];
			game.addGlobalSkill(event.target.playerid+'_rule');
		...
		ai:{
			result:{
				player:(player)=>{
					if(typeof player.storage.ykxuyan!='number') return 0;
					else return (player.maxHp-player.hp)*2+player.storage.ykxuyan/2-5;
				},
			},
			order:6,
			threaten:1.3,
	...
	lib.skill.ykfeibing={
		group:['ykfeibing_start'],
		enable:"phaseUse",
		usable:2,
		selectTarget:1,
		filterTarget:(card,player,target)=>{
			if(!player.ykCheckConsume('Mp',60)) return false;
			return target.storage.ykfeibing_mark&&target.storage.ykfeibing_mark.length;
		},
		content:function (){
			'step 0'
			event.target=target;
			event.card=target.storage.ykfeibing_mark[0];
			player.chooseTarget('选择【杀】的目标',function(card,player,target){
				return event.target.inRange(target)&&!target.storage.ykfeibing_mark;
			})
			'step 1'
			if(result.bool){
				player.ykConsume('Mp',60);
				event.target2=result.targets[0];
				event.target.useCard(game.createCard('sha',event.card.suit,event.card.num,event.card.nature),event.target2,false);
				event.target.line(event.target2);
				event.target.lose(event.card);
				event.target.removeSkill('ykfeibing_mark');
				event.target2.storage.ykfeibing_mark=[];
				event.target2.addSkill('ykfeibing_mark');
				event.target2.storage.ykfeibing_mark=event.target2.storage.ykfeibing_mark.concat([event.card]);
				if((lib.config['yk_ykfeibing_rank']||0)>=1&&!player.ykCheckConsume('Defend',player.ykGetMaxType('Defend')/2)) player.ykRecover('Defend',50);
				if((lib.config['yk_ykfeibing_rank']||0)>=2&&player.countCards('h')<player.maxHp) player.draw();
				if((lib.config['yk_ykfeibing_rank']||0)>=3&&!player.hujia) player.changeHujia();
				if((lib.config['yk_ykfeibing_rank']||0)>=4&&player.hp<2) player.recover();
				if((lib.config['yk_ykfeibing_rank']||0)>=5&&player.hp<event.target2.hp) event.target2.damage();
			}
		},
		subSkill:{
			start:{
				...
				filter:(event,player)=>{
					for(var pl of game.players) if(pl.storage.ykfeibing_mark) return false;
				...
				content:()=>{
					...
					if(result.bool){
						...
						player.lose(card);
						player.storage.ykfeibing_mark=[];
						lib.skill.ykfeibing_mark={
							init:player=>{
								player.markSkill('ykfeibing_mark');
							},
							mark:true,
							marktext:"兵",
							intro:{
							...
							onremove:player=>{
								delete player.storage.ykfeibing_mark;
								player.unmarkSkill('ykfeibing_mark');
							},
							enable:'phaseUse',
							filter:(event,player)=>{
								return player.storage.ykfeibing_mark&&player.storage.ykfeibing_mark.length&&!player.hasSkill('ykfeibing');
							},
							filterTarget:(card,player,target)=>{
								return !target.storage.ykfeibing_mark&&player.inRange(target)&&target!=player;
							},
							selectTarget:1,
							content:()=>{
								event.card=player.storage.ykfeibing_mark[0];
								player.lose(event.card);
								player.useCard(game.createCard('sha',event.card.suit,event.card.num,event.card.nature),target,false);
								player.removeSkill('ykfeibing_mark');
								target.storage.ykfeibing_mark=[];
								target.addSkill('ykfeibing_mark');
								target.storage.ykfeibing_mark=target.storage.ykfeibing_mark.concat([event.card]);
								if(player.name=='qxq_yk_lingshang'){
									if((lib.config['yk_ykfeibing_rank']||0)>=1&&!player.ykCheckConsume('Defend',player.ykGetMaxType('Defend')/2)) player.ykRecover('Defend',50);
									if((lib.config['yk_ykfeibing_rank']||0)>=2&&player.countCards('h')<player.maxHp) player.draw();
									if((lib.config['yk_ykfeibing_rank']||0)>=3&&!player.hujia) player.changeHujia();
									if((lib.config['yk_ykfeibing_rank']||0)>=4&&player.hp<2) player.recover();
									if((lib.config['yk_ykfeibing_rank']||0)>=5&&player.hp<target.hp) target.damage();
								}
							},
							ai:{
								order:get.order({name:'sha'})+0.1,
						...
						lib.translate.ykfeibing_mark='兵';
						player.addSkill('ykfeibing_mark');
						player.storage.ykfeibing_mark=player.storage.ykfeibing_mark.concat([card]);
					}
				},
			},
		},
	...
	lib.skill.ykduoren={
		init:player=>{
			var bool=true;
			for(var f of player.getFriends()) if(f.ykgroupTag) bool=false;
			if(player.name=='qxq_yk_lingshang'&&player.group=='qxq_yk'&&bool){
				player.ykgroupTag='qxq_yk_lingshang';
				lib.skill.ykduoren_check={
					trigger:{
						player:['ykfeibingAfter','useSkillAfter'],
					},
					forced:true,
					silent:true,
					popup:false,
					firstDo:true,
					filter:(event,player)=>{
						if(event.name=='useSkill'&&event.skill!='ykfeibing_mark') return false;
						var skill_source;
						for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_lingshang'){skill_source=pl;break;}
						if(skill_source) return true;
						return false;
					},
					content:()=>{
						var skill_source;
						for(var pl of game.players.concat(game.dead)) if(pl.ykgroupTag=='qxq_yk_lingshang'){skill_source=pl;break;}
						if(skill_source) skill_source.ykchangeWitchcraftPoint(1,'stone');
					},
				};
				game.addGlobalSkill('ykduoren_check');
				player.showWitchcraftDiv(5,()=>{
					"step 0"
					lib.skill.ykduoren.skillAnimation=true;
					lib.skill.ykduoren.animationColor="yellow";
					player.useSkill('ykduoren')._triggered=null;
					"step 1"
					game.broadcastAll(function(){
						game.ykSetImage2("https://gitee.com/qxqdpcq/yunkong/raw/master/extension/ykduoren_bg.jpg",ui.background);
					});
					game.removeGlobalSkill('ykduoren_check');
					player.ykrevive=lib.element.player.revive;
					player.ykrecover=lib.element.player.recover;
					player.ykphase=lib.element.player.phase;
					var numx=1;
					...
					player.draw(numx)._triggered=null;
					player.gainMaxHp(Math.floor(player.maxHp/2))._triggered=null;
					player.ykrecover(Infinity)._triggered=null;
					player.phase()._triggered=null;
					lib.skill.ykduoren.content=()=>{
						var n=1;
						if(Math.random()<0.75) n++;
						var cards=get.cards(n);
						player.gain(cards);
						if((lib.config['yk_ykduoren_rank']||0)>=4){
							var list=[];
							for(var c of cards) list.push(get.type(c));
							var card=get.cardPile2(function(card){
								return list.indexOf(get.type(card))==-1;
							});
							if(card) player.gain(card);
						}
					};
					if((lib.config['yk_ykduoren_rank']||0)>=1){
						player.ykRecover('Mp',Infinity);
						player.ykRecover('Defend',Infinity);
					}
					if((lib.config['yk_ykduoren_rank']||0)>=2) player.draw();
					if((lib.config['yk_ykduoren_rank']||0)>=3) player.changeHujia();
					if((lib.config['yk_ykduoren_rank']||0)>=5) player.gainMaxHp();
					delete lib.skill.ykduoren.skillAnimation;
					delete lib.skill.ykduoren.animationColor;
				},lib.translate.ykduoren_info,'stone');
			}
			delete lib.skill.ykduoren.init;
		},
		trigger:{
			global:['ykfeibingAfter','ykfeibing_markAfter'],
		},
		forced:true,
		content:()=>{
			player.draw();
	...
	lib.skill.ykxuandao={
		...
		intro:{
			name:'玄道',
			mark:function(dialog,storage,player){
				dialog.addText('拥有的武将牌');
				if(!player.storage.ykxuandao.length) dialog.addText('无');
				else dialog.addSmall([player.storage.ykxuandao,'character']);
			},
		},
		trigger:{
			player:["damageBegin","phaseBegin","ykzhixinAfter"],
			global:"gameStart",
		},
		filter:(event,player)=>{
			if(event.name=='phase') return player.storage.ykxuandao.length;
			else return !player.storage.ykxuandao.length;
		...
		content:()=>{
			...
			if(trigger.name!='phase'){
				...
				var list=[];
				for(var character in lib.character) if(lib.character[character][1]!='qxq_yk') list.push(character);
				player.storage.ykxuandao=list.randomGets(3);
				if((lib.config['yk_ykxuandao_rank']||0)>=1) player.ykRecover('Defend',30);
				if((lib.config['yk_ykxuandao_rank']||0)>=2) player.ykRecover('Mp',50);
				if((lib.config['yk_ykxuandao_rank']||0)>=4) player.draw();
				if((lib.config['yk_ykxuandao_rank']||0)>=5&&Math.random()<0.5) player.changeHujia();
				
			}
			else{
				player.chooseButton(['选择一张武将牌并获得上面的所有技能直到下个回合开始',[player.storage.ykxuandao,'character']],true).set('ai',button=>{
					if(lib.rank.rarity.legend.indexOf(button.link.derivation)) return 4;
					if(lib.rank.rarity.epic.indexOf(button.link.derivation)) return 3;
					if(lib.rank.rarity.rare.indexOf(button.link.derivation)) return 2;
					if(lib.rank.rarity.junk.indexOf(button.link.derivation)) return 1;
					return 1.5;
				});
			}
			'step 1'
			if(result.bool){
				event.result=result.links[0];
				if(Array.isArray(lib.character[event.result][3])) for(var skill of lib.character[event.result][3]) player.addTempSkill(skill,{player:"phaseBefore"});
			}
		},
		group:['ykxuandao_respond'],
		subSkill:{
			respond:{
				trigger:{
				...
				filter:function(event,player){
					if(event.name=='dying') return player.storage.ykxuandao.length;
					if(event.responded) return false;
					if(event.filterCard({name:'shan'},player,event)) return player.storage.ykxuandao.length;
				},
				content:()=>{
					'step 0'
					player.chooseButton(['选择要移除的武将牌',[player.storage.ykxuandao,'character']],true).set('ai',button=>{
						if(lib.rank.rarity.legend.indexOf(button.link.derivation)) return 1;
						if(lib.rank.rarity.epic.indexOf(button.link.derivation)) return 2;
						if(lib.rank.rarity.rare.indexOf(button.link.derivation)) return 3;
						if(lib.rank.rarity.junk.indexOf(button.link.derivation)) return 4;
						return 2.5;
					});
					'step 1'
					if(result.bool){
						var character=result.links[0];
						result=null;
						player.storage.ykxuandao.remove(character);
						player.update();
						player.updateMark();
						player.updateMarks();
						event.finish();
						if(trigger.name=='dying'){
							player.useCard(game.createCard('jiu'),player);
						}
						else if(trigger.filterCard({name:'shan'},player,trigger)){
						...
						event.list=[];
						if(Array.isArray(lib.character[character][3])) for(var skill of lib.character[character][3]) event.list.push('【'+get.translation(skill)+'】：'+(lib.translate[skill+'_info']?lib.translate[skill+'_info']:'暂无技能介绍'));
						if(event.list.length&&player.ykCheckConsume('Mp',((lib.config['yk_ykxuandao_rank']||0)>=3?100:120))) player.chooseControlList(event.list).set('ai',function(event,player){
							var compare=[];
							for(var skill of event.list) compare.push({
								name:skill,
								score:get.skillRank(skill,'in',true)||0.01,
					...
					'step 2'
					if(result&&typeof result.index=='number'){
						player.ykConsume('Mp',((lib.config['yk_ykxuandao_rank']||0)>=3?100:120),true);
						event.skill=event.list[result.index];
						if(event.skill) player.addSkill(event.skill);
					}
				},
			},
		},
	...
	lib.skill.ykzhixin={
		...
		forced:true,
		filter:(event,player)=>{
			return !player.storage.ykxuandao.length&&player.maxHp>2;
		},
		content:()=>{
			player.revive();
			player.loseMaxHp();
			trigger.cancel();
			if((lib.config['yk_ykzhixin_rank']||0)>=1) player.draw(2);
			if((lib.config['yk_ykzhixin_rank']||0)>=2) player.changeHujia();
			if((lib.config['yk_ykzhixin_rank']||0)>=4) player.ykRecover('Defend',Infinity);
			if((lib.config['yk_ykzhixin_rank']||0)>=5) player.recover(2);
			player.storage.ykxuandao=[];
			player.markSkill('ykxuandao');
			trigger.trigger('ykzhixin');
		},
		group:['ykzhixin_use'],
		subSkill:{
			use:{
				enable:"phaseUse",
				filter:(event,player)=>{
					if(player.ykCheckConsume('Mp',((lib.config['yk_ykzhixin_rank']||0)>=3?40:70))) return player.storage.ykxuandao.length&&!player.countCards('h');
				},
				content:()=>{
					'step 0'
					player.chooseButton(['选择要移除的武将牌',[player.storage.ykxuandao,'character']],true).set('ai',button=>{
					...
					'step 1'
					if(result.bool){
						player.ykConsume('Mp',((lib.config['yk_ykzhixin_rank']||0)>=3?40:70),true);
						var character=result.links[0];
						player.storage.ykxuandao.remove(character);
						player.update();
						player.updateMark();
						player.updateMarks();
						player.useCard(game.createCard('wuzhong'),player);
					}
				},
			},
		},
	...
	lib.translate['ykshimeng_info']='<a style="color:purple" href=\'javascript:alert("满足一定条件后触发的强力技能，同一个阵营最多只生效一个通灵技的通灵效果");\'>通灵技</a>，每当友方角色使用或打出转化牌时，获得1通灵点，通灵点数达到10时，通灵“月蚀神”：将以【月魇】获得的技能使用次数调整为每回合五次，生命上限翻倍并回复至满体力，摸两张牌（若为阵亡状态则复活并改为摸四张牌），立即获得一个额外的完整回合，将技能【月魇】和【蚀梦】替换为<a style="color:purple" href=\'javascript:alert("【月颂】：所有其他友方角色生命上限增加50%（向下取整）；锁定技，回合内，你使用转化后的基本/锦囊牌时，可消耗150术法值并弃置一张牌令此牌额外结算一次，你使用转化后的延时锦囊/装备牌时，可消耗150术法值并弃置一张牌令此牌额外增加一个使用目标。");\'>【月颂】</a>和<a style="color:purple" href=\'javascript:alert("【极梦】：你的手牌上限始终等于体力上限，当你需要使用或打出卡牌时，消耗50/140/100/200元力值将一张牌当作任意基本/锦囊/延时锦囊/装备牌使用或打出，非梦境状态下，当你失去最后一张手牌时，任意非友方角色回合内，你不会成为任意牌和技能的目标，直到你下个回合开始前或重新拥有手牌，若以前者方式移除，你摸一张牌。");\'>【极梦】</a>，并释放<a style="color:purple" href=\'javascript:alert("【梦境效果】：第一个阵亡的友方角色复活并回复至满体力，摸四张牌并结束梦境；梦境存在期间额外获得以下效果：1、任意非友方角色回合内，你不会成为任意牌和技能的目标（对自己使用酒或桃除外）；2、任意友方角色回合开始时，你回复50点元力值，若该回合为你的回合，此回复效果增加100%；3、你的所有元力值消耗减少50%，你的所有元力值恢复效果增加25%。，每当除你外的友方角色失去最后一张手牌时，其立即摸一张牌。");\'>【梦境效果】</a>。';
	...
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykshimeng_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'蚀梦Lv1.通灵时增加真气值及上限500点，增加元力值及上限500点</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykshimeng_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'蚀梦Lv2.技能【月颂】术法值消耗减少70点</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykshimeng_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'蚀梦Lv3.通灵时立即将真气值和元力值回复至上限</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykshimeng_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'蚀梦Lv4.技能【极梦】元力值消耗减少50%</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykshimeng_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'蚀梦Lv5.通灵后解除以【魇梦】获得的技能的次数限制，同时在梦境结束时仍保留友方和自己回合开始时的相应元力值回复效果，但该回复量减少50%</font>';
	lib.translate['ykyueyan']='月魇';
	lib.translate['ykyueyan_info']='游戏开始时，从技能池中随机抽取5个转化技，并可选择一名除你外的其他角色，令你和该角色获得其中一项技能，你/该角色每次发动此技能，你/该角色摸一张牌，你和其以此法获得的技能每回合限用两次，若未选择技能则你可从以下四个技能中获取一个：1、出牌阶段限两次，你可以将基本牌当作任意基本牌使用或打出；2、出牌阶段限两次，你可以将锦囊牌当作任意锦囊牌使用或打出；3、出牌阶段限两次，你可以将延时锦囊牌当作任意延时锦囊牌使用或打出；4、出牌阶段限两次，你可以将装备牌当作任意装备牌使用或打出；以上所有以【月魇】方式获得的技能每次发动均消耗80元力值，并获得30真气值';
	...
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykyueyan_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'月魇Lv1.真气值获得增加20点</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykyueyan_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'月魇Lv2.元力值消耗降低30点</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykyueyan_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'月魇Lv3.每次发动该技能，有10%概率获得一点护甲</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykyueyan_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'月魇Lv4.每次发动该技能，有10%概率额外摸一张牌</font>';
	lib.translate['qxq_yk_yanmengyuejian_ykIntroduce']+='<br>'+((lib.config['yk_ykyueyan_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'月魇Lv5.该技能限制次数改为3次</font>';
	...
	lib.translate['ykpiaoling_info']='出牌阶段，将任意张红桃牌交给一名其他角色，每交出一张牌，回复60术法值和30元力值，每交出五张牌，你立即摸两张牌，且目标立即摸一张牌，若牌数超过你当前的体力值，则你获得【寄篱】直到你的下个回合开始前。<br><b>【寄篱】</b>：锁定技，任意以你为目标的牌，将此牌的目标转移给该角色；该角色获得牌时，需将牌数量的一半（向下取整）分给你。';
	...
	lib.translate['qxq_yk_xiaoqiao_ykIntroduce']+='<br>'+((lib.config['yk_ykpiaoling_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'飘零Lv5.你获得【寄篱】时，可选择摸三张牌并将武将牌翻面</font>';
	...
	lib.translate['ykjuejian_info']='<small><i>————剑既出，断无收回之理。</i></small><br>你有武器牌且使用“杀”时，可以消耗35术法值，令此“杀”无视防具。';
	...
	lib.translate['qxq_yk_wuwangxuanyue_ykIntroduce']+='<br>'+((lib.config['yk_ykshilie_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'时裂Lv1.此技能术法值消耗减少100点</font>';
	lib.translate['qxq_yk_wuwangxuanyue_ykIntroduce']+='<br>'+((lib.config['yk_ykshilie_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'时裂Lv2.发动此技能时，清除目标300点真气值</font>';
	...
	//秩序
	lib.translate['ykxuyan']='序演';
	lib.translate['ykxuyan_info']='锁定技，其他角色回合开始时，从牌堆中抽取(x+场上角色数)张卡牌（最多不超过x+4张），你可以用手牌任意替换之，然后将其置于该角色的武将牌上，视为“秩”，若目标直到其回合结束前打出了与“秩”牌名相同的牌，你可消耗75术法值，选择以下一项：1、令此牌失效；2、令此牌额外结算一次；若如此做，则直到其下一个回合开始前，此技能对其不再生效，且你获得一个“序”（“序”上限为7个，x为你已损失体力值）。';
	lib.translate['qxq_yk_zhixu_ykIntroduce']=((lib.config['yk_ykxuyan_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'序演</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykxuyan_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'序演Lv1.此技能消耗术法值减少10点</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykxuyan_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'序演Lv2.发动此技能时，15%概率摸一张牌</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykxuyan_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'序演Lv3.发动此技能时，回复20点真气值</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykxuyan_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'序演Lv4.发动此技能时，回复15点术法值</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykxuyan_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'序演Lv5.发动此技能时，15%概率回复一点体力</font>';
	lib.translate['ykdingze']='定则';
	lib.translate['ykdingze_info']='每当你获得一个序，你可消耗150术法值，从下列规则内永久增加一条规则：1、回合内对自己角色使用红桃牌时，你回复一点体力；2、回合内对自己使用方块牌时，你摸两张牌；3、回合内对目标使用梅花牌时，你弃置一张牌，目标弃置三张牌；4、回合内对目标使用黑桃牌时，目标摸一张牌并翻面（以上效果回合内最多共计生效(X-4)次，X为“序”的数量，至少生效一次）。';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br><br>'+((lib.config['yk_ykdingze_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'定则</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykdingze_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'定则Lv1.此技能消耗术法值减少20点</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykdingze_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'定则Lv2.发动此技能时，回复25点真气值</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykdingze_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'定则Lv3.游戏开始时，术法值及术法值上限增加300点</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykdingze_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'定则Lv4.取消黑桃牌对目标的摸牌效果</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_ykdingze_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'定则Lv5.取消梅花牌对自己的弃牌效果</font>';
	lib.translate['yktianzhi']='天秩';
	lib.translate['yktianzhi_info']='限定技，消耗300术法值，出牌阶段，令所有其他角色依次从“规则集”内挑选一条规则并令其共同遵守直到游戏结束（规则集内规则数量为2X，X为“序”的数量）。';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br><br>'+((lib.config['yk_yktianzhi_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'天秩</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_yktianzhi_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'天秩Lv1.发动此技能时，回复满真气值</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_yktianzhi_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'天秩Lv2.发动此技能时，回复满体力值</font>';
	lib.translate['qxq_yk_zhixu_ykIntroduce']+='<br>'+((lib.config['yk_yktianzhi_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'天秩Lv3.发动此技能时，回复满术法值</font>';
	...
	//翎上
	lib.translate['ykfeibing']='飞兵';
	lib.translate['ykfeibing_info']='游戏开始时，额外摸一张牌并将一张牌置于你的武将牌上，称为“兵”，拥有“兵”的武将，其的出牌阶段可以将“兵”对没有“兵”的其他角色使用，当作一张不计入出杀次数的【杀】，此【杀】花色、数字和属性均与“兵”相同，且结算后，目标获得此“兵”并置于其武将牌上；你的出牌阶段，你可以调动两次场上的“兵”，每次消耗60术法值。';
	lib.translate['qxq_yk_lingshang_ykIntroduce']=((lib.config['yk_ykfeibing_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'飞兵</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykfeibing_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'飞兵Lv1.你发动此技能时，若你的真气值小于上限的一半，则你回复50真气值</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykfeibing_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'飞兵Lv2.你发动此技能时，若你手牌数小于体力上限的一半，则你摸一张牌</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykfeibing_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'飞兵Lv3.你发动此技能时，若你没有护甲，则你获得一点护甲</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykfeibing_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'飞兵Lv4.你发动此技能时，若你的体力值小于2，则你回复一点体力</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykfeibing_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'飞兵Lv5.你发动此技能时，若你的体力值小于目标，则目标受到一点伤害</font>';
	lib.translate['ykduoren']='夺刃';
	lib.translate['ykduoren_info']='<a style="color:yellow" href=\'javascript:alert("满足一定条件后触发的强力技能，同一个阵营最多只生效一个通灵技的通灵效果");\'>通灵技</a>，场上“兵”移动时，你摸一张牌并获得1通灵点，通灵点数达到5时，摸一张牌并立即获得一个回合（若阵亡则复活并改为摸三张牌），体力上限增加50%，将体力回复至上限，同时将【夺刃】效果改为触发时摸一张牌并有75%概率额外摸一张牌。';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br><br>'+((lib.config['yk_ykduoren_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'夺刃</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykduoren_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'夺刃Lv1.通灵时，将术法值和真气值回复至上限</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykduoren_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'夺刃Lv2.通灵时，额外摸一张牌</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykduoren_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'夺刃Lv3.通灵时，获得一点护甲</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykduoren_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'夺刃Lv4.通灵后触发【夺刃】额外从牌堆中获得一张与两张牌类型均不同的牌</font>';
	lib.translate['qxq_yk_lingshang_ykIntroduce']+='<br>'+((lib.config['yk_ykduoren_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'夺刃Lv5.通灵时额外增加一点体力上限</font>';
	
	//帝君玄牝
	lib.translate['ykxuandao']='玄道';
	lib.translate['ykxuandao_info']='锁定技，每当你即将受到伤害时，若你的武将牌上没有任何武将牌，你流失两点体力并取消之，然后抽取三张武将牌并置于你的武将牌上，你的回合开始时，从中选取一张武将牌并获得上面的所有技能直到你下个回合开始，当你需要使用或打出【闪】时，你可以将以此法获得的武将牌当作【闪】使用或打出，当你濒死时，你可以将以此法获得的武将牌当作“酒”使用或打出，每当你以此技能打出一张武将牌，你可以消耗120术法值，永久获得此武将牌上面的一项技能；游戏开始时，发动一次无消耗的【玄道】。';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']=((lib.config['yk_ykxuandao_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'玄道</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykxuandao_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'玄道Lv1.每次获得武将牌，回复30点真气值</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykxuandao_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'玄道Lv2.每次获得武将牌，回复50点术法值</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykxuandao_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'玄道Lv3.消耗的术法值减少20点</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykxuandao_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'玄道Lv4.每次获得武将牌，额外摸一张牌</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykxuandao_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'玄道Lv5.每次获得武将牌，50%概率获得一点护甲</font>';
	lib.translate['ykzhixin']='止心';
	lib.translate['ykzhixin_info']='锁定技，你即将阵亡时，若你的武将牌上没有任何武将牌且体力上限大于2，则你减一点体力上限取消之，并发动一次【玄道】，出牌阶段限一次，当你没有手牌时，你可以消耗70术法值，将武将牌上的一张武将牌当作【无中生有】使用或打出。';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br><br>'+((lib.config['yk_ykzhixin_rank']||0)>=1?'<font color=orange>':'<font color=grey>')+'止心</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykzhixin_rank']||0)>=1?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'止心Lv1.复活时额外摸两张牌</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykzhixin_rank']||0)>=2?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'止心Lv2.复活时额外获得一点护甲</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykzhixin_rank']||0)>=3?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'止心Lv3.消耗的术法值减少30点</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykzhixin_rank']||0)>=4?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'止心Lv4.复活时将真气值回复至上限</font>';
	lib.translate['qxq_yk_dijunxuanpin_ykIntroduce']+='<br>'+((lib.config['yk_ykzhixin_rank']||0)>=5?'<font color=orange>（已激活）':'<font color=grey>（未激活）')+'止心Lv5.复活时回复两点体力值</font>';
	...
	if(!game.ykHasCharacter('qxq_yk_dijunxuanpin')){
		lib.skill.ykxuandao={};
		delete lib.skill.ykxuandao;
		lib.skill.ykzhixin={};
		delete lib.skill.ykzhixin;
	}
});