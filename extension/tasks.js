...
window.YKimport(function(lib,game,ui,get,ai,_status){
	lib.ykTask={
		daily:{
			turntable:{
				...
				info:'每日转盘抽奖',
				filter:()=>{
					return lib.config.yk_turntable_record==window.playTime.years+'/'+window.playTime.months+'/'+window.playTime.days;
				},
				content:()=>{
					window.ykOpenTask();
					window.turntable([{
						...
						name:'yk_fan_bookPage',
						p:5,
						info:'【秘籍残页·凡阶】x3',
						func:()=>{
							game.yk_gainItem('yk_fan_bookPage',3);
							alert('获得【秘籍残页·凡阶】x3！');
						},
					},{
						name:'yk_huang_bookPage',
						p:3,
						info:'【秘籍残页·黄阶】x3',
						func:()=>{
							game.yk_gainItem('yk_huang_bookPage',3);
							alert('获得【秘籍残页·黄阶】x3！');
						},
					},{
						name:'yk_redbag',
						...
						info:'【红包】x1',
						func:()=>{
							game.yk_gainItem('yk_redbag',1);
							alert('获得【红包】x1！');
						},
					},{
						name:'upGradeSkill_stone',
						p:2,
						info:'【随机技能石】x1',
						func:()=>{
						...
						game.saveConfig('yk_turntable_record',window.playTime.years+'/'+window.playTime.months+'/'+window.playTime.days);
						window.ykOpenTask();
					});
				},
				onover_func:result=>{
					...
					for(var i=1;i<num+1;i++){
						...
						if(lib.config.yk_signIn_date.days.indexOf(i)==-1&&(''+window.playTime.days)!=(''+i)){
							...
							divx.onclick=function(){
								...
								game.saveConfig('yk_signIn_date',lib.config.yk_signIn_date);
								this.source.delete();
								game.yk_gainItem('sky_crying',300);
								alert('签到成功，获得【虚空之泪】x300！');
			...
			challenge:{
				...
				content:()=>{
					...
					if(!lib.config.weeklyChallenge_gainReward[order]&&lib.config.weeklyChallenge_Bool[order]){
						...
						game.yk_gainItem('sky_crying',750);
						alert('领取成功，获得【虚空之泪】x750！');
						window['choose_每日任务'].onclick();
					}
	...
	window.ykOpenTask=function(){
		if(typeof window.ykCloseTask=='function'){
			window.ykCloseTask();
			window.ykCloseTask=null;
			return ;
		}
		var div=ui.create.div('.menu');
		div.style.cssText='height:400px;width:500px;top:calc( 50% - 200px );left:calc( 50% - 250px );z-index:9999;border-radius:8px;';
		ui.window.appendChild(div);
		window.ykCloseTask=function(){
			div.delete();
		};
		var button=ui.create.div('.menubutton.round','×',window.ykCloseTask);
		button.style.cssText='top:5px;left:calc(100% - 55px);z-index:99999;';
		div.appendChild(button);
		window.yk_clickFK(button);
