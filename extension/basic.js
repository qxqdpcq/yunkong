...
// ---------------------------------------适配千幻用------------------------------------------//
	...
	game.YKgainNewPerson=function(characterx,bool){
		...
		if(characterx.length<=0&&confirm('新武将重启后才会显示于武将列表，且新武将的角色图片需要前往【自动检测版本和更新】处下载！是否立即重启？')){
			if(game.sayyk&&typeof game.sayyk=='function') game.sayyk('恭喜获得角色：【'+lib.translate[name]+'】！即将重启以保存。');
			setTimeout(function(){game.reload();},3000);
		}
		else if(game.sayyk&&typeof game.sayyk=='function') game.sayyk('恭喜获得角色：【'+lib.translate[name]+'】！');
	}
	...
	lib.ykDrawCardPoolSchel={
		...
		yk_yongmengyixin:[[2022,9,18],[2022,10,3],'<span style=\"color: #FF00FF;\">梦想</span><small><u>花开时</u></small>',['qxq_yk_fuling'],['qxq_yk_akalai'],[]],
